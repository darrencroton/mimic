/**
 * @file    test_unit_horizontal_reader_realdata.c
 * @brief   Opt-in unit test of the horizontal_hdf5 reader against the full dataset.
 *
 * Opens the complete 50-snapshot micro-Uchuu conversion through the horizontal
...
int test_open_run_against_full_dataset(void) {
  ...
  TEST_ASSERT(horizontal_identity_bounds_valid(&info, MimicConfig.UniqueGalaxyIDMultiplier) != 0,
...
int main(void) {
  printf("%s", BLUE);
  printf("============================================================\n");
  printf("Test Suite: Horizontal Reader (full dataset, opt-in)\n");
  printf("============================================================\n");
  printf("%s\n", NC);
