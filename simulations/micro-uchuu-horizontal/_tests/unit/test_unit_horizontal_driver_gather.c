...
static struct HorizontalHaloAux previous_aux[NHALOS];
static struct HorizontalHaloAux current_aux[NHALOS];
...
static int test_progenitor_count_uses_the_previous_generation(void) {
  ...
  TEST_ASSERT_EQUAL((int)horizontal_count_progenitor_galaxies(view, &prev, 0), 3,
                    "the count should sum slab N-1's output ranges along its own chain");
  /* p4 contributes 1, p5 contributes 3. */
  TEST_ASSERT_EQUAL((int)horizontal_count_progenitor_galaxies(view, &prev, 1), 4,
                    "the second chain's ranges should sum to 4");

  TEST_ASSERT(horizontal_count_progenitor_galaxies(view, &transposed, 0) !=
                  horizontal_count_progenitor_galaxies(view, &prev, 0),
...
static int test_gather_order_sources_and_times(void) {
  ...
  horizontal_gather_progenitor_galaxies(view, &prev, 0, first_occupied, gathered);
...
static int test_gather_reads_the_previous_output_buffer(void) {
  ...
  memset(gathered, 0, sizeof(gathered));
  horizontal_gather_progenitor_galaxies(
      view, &prev, 0, horizontal_find_most_massive_progenitor(view, &prev, 0), gathered);
...
int main(void) {
  printf("%s", BLUE);
  printf("============================================================\n");
  printf("Test Suite: Horizontal Driver Cross-Generation Gather\n");
  printf("============================================================\n");
  printf("%s\n", NC);
