#!/usr/bin/env python3
"""
Cross-format identity gate for the horizontal driver.

The same simulation, read through two different drivers, must produce the same
galaxies. This harness runs micro-Uchuu through the vertical driver
(``micro-uchuu-ascii``, Consistent-Trees ASCII) and through the horizontal
driver (``micro-uchuu-horizontal``, snapshot HDF5) and requires, for every output
...
2. Run-file diffs -- each committed horizontal run file differs from its ascii
...
4-7. Identity -- ``halos-only`` first (fixed timesteps, then dynamic), then
   ...
   of each run (the vertical side writes five; comparing one file would silently
   compare a fifth of the run).
8. Vertical-path preservation -- the same vertical run built from the
...
HORIZONTAL_SIMULATION = "micro-uchuu-horizontal"
...
def stage_preconditions():
    ...
    alist = REPO_ROOT / "simulations" / HORIZONTAL_SIMULATION / "micro-uchuu.a_list"
    ...
    snapshot_data = assert_dataset_present(HORIZONTAL_SIMULATION, required)
    log(
        f"  {HORIZONTAL_SIMULATION}: {snapshot_data} -> {snapshot_data.resolve()} "
...
def assert_matches_head(path: Path) -> None:
    ...
    working-vertical run file would never reach the runs -- but it would still be
...
def split_leading_header(lines: list[str]) -> tuple[list[str], list[str]]:
    ...
    header would force the horizontal run file to describe itself as
...
def assert_snapshot_run_file_diff(ascii_path: Path, snapshot_path: Path) -> None:
    """The horizontal run file differs from the ascii one in exactly two keys.
...
def stage_run_file_diffs():
    ...
    for model in MODELS:
        ...
        snapshot_path = committed_run_file(model, HORIZONTAL_SIMULATION)
...
def stage_build_worktrees():
    ...
    for model in MODELS:
        for simulation in (ASCII_SIMULATION, HORIZONTAL_SIMULATION):
...
def preflight(tree_run: RunOutput, horizontal_run: RunOutput) -> tuple[int, int]:
    ...
    """
    assert_alist_byte_equal(alist_used_by(tree_run), alist_used_by(horizontal_run))
    log("  preflight: snapshot lists byte-equal")

    tree_attrs, tree_z, tree_fields = run_properties(tree_run.master)
    snap_attrs, snap_z, snap_fields = run_properties(horizontal_run.master)
    ...
    for name in PREFLIGHT_ATTRS:
        for attrs, run in ((tree_attrs, tree_run), (snap_attrs, horizontal_run)):
    ...
    # horizontal package's is "applied by the producer before emission"). Nothing
    ...
    snap_signature = record_signature(horizontal_run)
    ...
    snap_records = recorded_records(horizontal_run)
    ...
    snapshot_expected = requested_snapshots(horizontal_run)
    ...
    assert_snapshot_coverage(horizontal_run, snap_records, expected)
...
def compare_pair(
    ...
    snapshot_partitions = len(horizontal_run.partitions())
    if tree_partitions < 2:
        raise AssertionError(
            f"{model}/{scheme}: the vertical run wrote {tree_partitions} partition file(s); "
    ...
    snapshot_requested = len(requested_snapshots(horizontal_run))
    if snapshot_partitions != snapshot_requested:
        raise AssertionError(
            f"{model}/{scheme}: the horizontal run wrote {snapshot_partitions} partition "
            f"file(s) for {snapshot_requested} requested output snapshot(s); a horizontal "
    ...
    log(
        ...
        f"(vertical {tree_partitions} partition(s) vs horizontal "
...
def identity_stage(model: str, scheme: str) -> None:
    ...
    records, fields = preflight(tree_run, horizontal_run)
    compare_pair(model, scheme, tree_run, horizontal_run, records, fields)
...
def stage_tree_path_preservation():
    ...
    GATE.require("identity:halos-only:fixed", "no HEAD vertical run to compare against")
