...
UniqueGalaxyID Design (from halo_properties.yaml):
  ...
  - Formula: creation_halonr + 10^9 * (forestnr_global + 1)
    ...
    if n_unique < n_snap62:
        ...
        for i, (idx, count) in enumerate(sorted(duplicated_indices.items())[:10]):
            ...
            for j, halo in enumerate(dup_halos[:5]):  # Show first 5
                print(
                    f"{RED}      Galaxy {j+1}: Type={halo.Type}, Mvir={halo.Mvir:.3f}, "
...
def test_uniquegalid_persistence():
    ...
    if missing:
        ...
        print(
            f"{YELLOW}  Note: No mergers should happen without modules, so this may indicate a bug or rare edge case.{NC}"
        )
        print(f"{YELLOW}  Showing first 10 missing galaxies:{NC}")
        for i, halo in enumerate(missing[:10]):
            print(
                f"{YELLOW}    UniqueGalaxyID {halo.UniqueGalaxyID}: Type={halo.Type}, Mvir={halo.Mvir:.3f}, "
...
def test_uniquegalid_physical_evolution():
    ...
    if large_changes:
        ...
        if infinite_changes > 0:
            print(
                f"{YELLOW}  Note: {infinite_changes} galaxies have infinite changes due to Vvir=0 in snapshot 62 (edge case){NC}"
            )
        print(f"{YELLOW}  Showing first 10 galaxies with large Vvir changes:{NC}")

        # Sort by relative change (largest first)
        large_changes.sort(
            key=lambda x: x["rel_change"] if x["rel_change"] != float("inf") else 1e10, reverse=True
        )

        for i, change_info in enumerate(large_changes[:10]):
            rel_change_str = (
                "inf"
                if change_info["rel_change"] == float("inf")
                else f"{change_info['rel_change']:.2f}"
            )
            rel_change_pct = (
                "inf%"
                if change_info["rel_change"] == float("inf")
                else f"{change_info['rel_change']*100:.1f}%"
            )

            print(f"{YELLOW}\n    {i+1}. UniqueGalaxyID {change_info['UniqueGalaxyID']}:{NC}")
            print(f"{YELLOW}       Type: {change_info['Type_62']} → {change_info['Type_63']}{NC}")
            print(
                f"{YELLOW}       Vvir: {change_info['Vvir_62']:.2f} → {change_info['Vvir_63']:.2f} km/s{NC}"
            )
            print(
                f"{YELLOW}       Mvir: {change_info['Mvir_62']:.3f} → {change_info['Mvir_63']:.3f} (1e10 Msun/h){NC}"
            )
