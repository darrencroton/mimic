...
def plot(
    ...
    for i in range(nbins - 1):
        ...
        if satellite_bin_count > 0:
            ...
            satellite_fraction_lo.append(
                quiescent_low_mass_count / satellite_bin_count if satellite_bin_count > 0 else 0.0
            )
            ...
            satellite_fraction_hi.append(
                quiescent_high_mass_count / satellite_bin_count if satellite_bin_count > 0 else 0.0
            )
        else:
    ...
    if np.any(mask):
        ax.plot(
            mass[mask],
            satellite_fraction_lo[mask],
            "r--",
            label="Satellites (groups/clusters)",
            linewidth=0.8,
        )
