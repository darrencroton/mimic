...
int sage_quasar_mode_init(void) {
  LOAD_AND_VALIDATE_RANGE_EXCLUSIVE("BlackHoleGrowthRate", BLACK_HOLE_GROWTH_RATE, 0.0, 1.0,
                                    "BH growth rate");
  LOAD_AND_VALIDATE_RANGE_INCLUSIVE("QuasarModeEfficiency", QUASAR_MODE_EFFICIENCY, 0.0, 1.0,
                                    "quasar mode efficiency");
  ...
  if (module_in_substep_phase("sage_quasar_mode", PROCESSING_MODE_PER_EVENT) &&
      !modules_in_same_substep_phase("sage_quasar_mode", PROCESSING_MODE_PER_EVENT,
...
int sage_quasar_mode_process(struct ModuleContext *ctx, struct Halo *halos, int ngal) {
  ...
  if (ctx->active_event != NULL) {
    ...
    const double bh_accrete =
        mimic_apply_black_hole_growth(event_halo, merger_ratio, BLACK_HOLE_GROWTH_RATE);
    if (bh_accrete > 0.0) {
      mimic_apply_quasar_mode_wind(event_halo, bh_accrete, QUASAR_MODE_EFFICIENCY, ctx);
    }

    DEBUG_LOG("Quasar mode from merger event (ratio=%.3f, source=%d, target=%d)", merger_ratio,
  ...
  if (gal->UnstableDiskGasFraction > 0.0) {
    ...
    if (BHaccrete > 0.0) {
      mimic_apply_quasar_mode_wind(halo, BHaccrete, QUASAR_MODE_EFFICIENCY, ctx);
    }
