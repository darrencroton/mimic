 ...
 * @brief   SAGE supernova feedback - computes SN reheating and ejection (swappable SN feedback
 * prescription)
 *
...
int sage_calculate_supernova_feedback_init(void) {
  LOAD_AND_VALIDATE_RANGE_INCLUSIVE("FeedbackReheatingEpsilon", FEEDBACK_REHEATING_EPSILON, 0.0,
                                    100.0, "reheating efficiency");
  ...
  if (module_configured_anywhere("sage_calculate_star_formation") &&
      !module_precedes_in_substep_phase("sage_calculate_star_formation", PROCESSING_MODE_BY_GALAXY,
...
int sage_calculate_supernova_feedback_process(struct ModuleContext *ctx, struct Halo *halos,
                                              int ngal) {
  if (ngal != 1) {
  ...
  DEBUG_LOG("Type=%d: Stars=%.3e, Reheat=%.3e, Eject=%.3e (CentralVvir=%.2f)", halo->Type, stars,
            reheated_mass, ejected_mass, central_vvir);
