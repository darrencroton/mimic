/**
 * @file    test_unit_sage_set_disk_scale_radius.c
 * @brief   Unit tests for sage_set_disk_scale_radius module
 *
 * Validates: Physics calculation logic, edge cases, parameter handling
 *
 * This test validates the sage_set_disk_scale_radius module physics:
 ...
 */
static void setup_test_galaxy(struct Halo *halo, struct GalaxyData *gal, float spin_x, float spin_y,
                              float spin_z, float vvir, float rvir, int type) {
...
int test_disk_radius_normal_halo(void) {
  ...
  int result = sage_set_disk_scale_radius_init();
  ...
  const float expected_rd = calculate_expected_disk_radius(spin_x, spin_y, spin_z, vvir, rvir);

  /* ===== EXECUTE ===== */
  result = sage_set_disk_scale_radius_process(&ctx, &halo, 1);
...
int test_disk_radius_high_spin(void) {
  ...
  /* ===== EXECUTE ===== */
  sage_set_disk_scale_radius_process(&ctx, &halo, 1);
...
int test_disk_radius_multiple_galaxies(void) {
  ...
  const float expected_rd0 = calculate_expected_disk_radius(100.0f, 100.0f, 100.0f, 200.0f, 0.2f);
  /* ===== EXECUTE ===== */
  ...
  TEST_ASSERT(gals[2].DiskScaleRadius == -1.0f, "Galaxy 2 (Type 2) should not be modified");
...
int test_module_initialization(void) {
  ...
  MimicConfig.pre_timestep[0].module_name = strdup("sage_set_disk_scale_radius");
 ...
 *
 * Executes all sage_set_disk_scale_radius unit tests and reports results.
 */
int main(void) {
  printf("%s", BLUE);
  printf("============================================================\n");
  printf("Test Suite: sage_set_disk_scale_radius Unit Tests\n");
  printf("============================================================\n");
  printf("%s", NC);
