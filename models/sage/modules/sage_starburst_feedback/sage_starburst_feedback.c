...
#include "sage/shared/sage_starburst_physics.h"
...
int sage_starburst_feedback_process(struct ModuleContext *ctx,
                                       struct Halo *halos,
                                       int ngal)
{
    ...
    if (ctx->active_event != NULL) {
        if (ngal != 1) {
            ERROR_LOG("sage_starburst_feedback (process_per_event) expects ngal=1, got %d",
                      ngal);
            return -1;
        }
