# User-Created Physics Utilities (models/shared/)
...
- It is a universal constant; use `module_system/physical_constants.h`
- It is a model parameter; read it from input YAML with parameter helpers

### Steps

1. **Create header file**: `models/shared/my_utility.h`
...
4. **Write tests**:
   - Create `models/shared/_tests/test_unit_my_utility.c`
   - Register it in `models/shared/module_info.yaml`
...
cp models/shared/my_model.h archive/shared-physics/my_model_v1.h

# Install the alternative implementation intentionally
cp alternative_implementation.h models/shared/my_model.h
...
1. **Create test file** in `models/shared/_tests/` directory:
```c
// models/shared/_tests/test_unit_my_utility.c
...
2. **Add to test registry** in `models/shared/module_info.yaml`:
```yaml
tests:
  unit:
...
### Include Paths
- **Use relative paths** - `#include "../models/shared/..."` makes dependencies explicit
- **Self-documenting** - You see exactly what file is being included

---

## Difference from module_system/

| Aspect | `module_system/` | `models/shared/` |
|--------|-----------|-----------|
...
All physics code (including shared utilities) lives in `models/sage/modules/`. The core (`src/core/`, `src/util/`, `src/io/`) remains **physics-agnostic**.
...
- [src/module_system/](../../src/module_system/README.md) - Framework infrastructure (constants, templates)
