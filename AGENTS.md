...
| Simulation package or vertical reader work | `mimic-simulations-and-readers`; add `mimic-properties` for `halo_properties.yaml` |
...
├── io/vertical/     Per-forest merger-tree readers   io/output/  Output writers
├── io/horizontal/   Per-snapshot slab reader
...
- **Reader/driver boundary** — `input.tree_type` selects the on-disk reader; `input.processing_order` selects the driver (`vertical` default, per-forest depth-first; `horizontal` per-snapshot in increasing time order). Startup validation rejects any reader/driver pair that disagrees.
