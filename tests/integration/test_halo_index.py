#!/usr/bin/env python3
"""
HaloIndex Validation Integration Test

Validates: HaloIndex uniqueness and persistence across snapshots
Phase: Core Property Validation
Scope: Physics-free mode (no modules), snapshots 62 and 63

HaloIndex Design (from halo_properties.yaml):
  - Every new galaxy gets a unique HaloIndex
  - HaloIndex is carried forward across all snapshots
  - Formula: file*10^12 + tree*10^6 + creation_halonr
  - Without mergers, every galaxy should exist in all future snapshots

Test Strategy:
  1. Check all HaloIndex values are unique in snapshot 62 (FAIL if duplicates)
  2. Check all galaxies in snap 62 exist in snap 63 (WARNING if missing)
  3. Check all HaloIndex values are unique in snapshot 63 (FAIL if duplicates)
...
def load_haloindex_test_data():
    """
    Load test data for HaloIndex validation.

    Runs Mimic with test_haloindex.yaml (outputs snapshots 62 and 63) and
    ...
    param_file = TEST_DATA_DIR / "test_haloindex.yaml"

    # Run Mimic if needed (generates both snapshot files)
    # Check for any model_haloindex files
    haloindex_files = list(output_dir.glob("model_haloindex_*"))
    if len(haloindex_files) < 2:
    ...
    all_halos = []
    for output_file in sorted(haloindex_files):
...
def test_haloindex_uniqueness_snapshot62():
    """
    Test 1: Check that all HaloIndex values are unique in snapshot 62.

    What: Validates every galaxy in snapshot 62 has a unique identifier
    Expected: No duplicates - FAIL if any found
    Rationale: HaloIndex must be unique within a snapshot to identify galaxies
    """
    print("Test 1: Checking HaloIndex uniqueness in snapshot 62...")
    ...
    # Load test data
    output_halos = load_haloindex_test_data()
    ...
    halo_indices = snap62_halos.HaloIndex
    ...
    if n_unique < n_snap62:
        ...
        print(f"\n{RED}  ✗ FAIL: Found {n_duplicates} duplicate HaloIndex values in snapshot 62!{NC}")
        ...
        print(f"\n{RED}  Duplicated HaloIndex values (showing first 10):{NC}")
        for i, (idx, count) in enumerate(sorted(duplicated_indices.items())[:10]):
            print(f"{RED}    HaloIndex {idx}: appears {count} times{NC}")
            # Show which halos have this index
            dup_mask = snap62_halos.HaloIndex == idx
        ...
        assert False, (
            f"HaloIndex uniqueness FAILED in snapshot 62! "
            f"Found {n_duplicates} duplicates across {len(duplicated_indices)} distinct values. "
            f"Every galaxy must have a unique HaloIndex."
        )

    print(f"{GREEN}  ✓ PASS: All {n_snap62} HaloIndex values are unique in snapshot 62{NC}")


def test_haloindex_persistence():
    """
    ...
    # Build lookup: HaloIndex -> halo for snapshot 63
    snap63_lookup = {}
    for halo in snap63_halos:
        halo_idx = halo.HaloIndex
        if halo_idx in snap63_lookup:
            # This should never happen if Test 1 and Test 3 pass
            print(f"{RED}  ✗ ERROR: Duplicate HaloIndex {halo_idx} in snapshot 63!{NC}")
    ...
    if missing:
        ...
        for i, halo in enumerate(missing[:10]):
            print(f"{YELLOW}    HaloIndex {halo.HaloIndex}: Type={halo.Type}, Mvir={halo.Mvir:.3f}, "
    ...
    snap62_index_set = set(snap62_halos.HaloIndex)
    new_in_63 = [h for h in snap63_halos if h.HaloIndex not in snap62_index_set]

    print(f"\n  New galaxies in snapshot 63: {len(new_in_63)}")
    print(f"    (These should have unique HaloIndex values - validated in Test 3)")

    print(f"{GREEN}  ✓ PASS: Persistence check complete ({match_pct:.1f}% matched){NC}")


def test_haloindex_uniqueness_snapshot63():
    """
    Test 3: Check that all HaloIndex values are unique in snapshot 63.
    ...
    """
    print("\nTest 3: Checking HaloIndex uniqueness in snapshot 63...")
    ...
    # Check for duplicates
    halo_indices = snap63_halos.HaloIndex
    unique_indices = np.unique(halo_indices)
    n_unique = len(unique_indices)
    ...
    if n_unique < n_snap63:
        ...
        print(f"\n{RED}  ✗ FAIL: Found {n_duplicates} duplicate HaloIndex values in snapshot 63!{NC}")
        ...
        for i, (idx, count) in enumerate(sorted(duplicated_indices.items())[:10]):
            print(f"{RED}    HaloIndex {idx}: appears {count} times{NC}")
            # Show which halos have this index
            dup_mask = snap63_halos.HaloIndex == idx
        ...
        assert False, (
            f"HaloIndex uniqueness FAILED in snapshot 63! "
            f"Found {n_duplicates} duplicates across {len(duplicated_indices)} distinct values. "
            f"Every galaxy must have a unique HaloIndex."
        )

    print(f"{GREEN}  ✓ PASS: All {n_snap63} HaloIndex values are unique in snapshot 63{NC}")


def test_haloindex_physical_evolution():
    """
    ...
    # Build lookup: HaloIndex -> halo for both snapshots
    snap62_lookup = {halo.HaloIndex: halo for halo in snap62_halos}
    snap63_lookup = {halo.HaloIndex: halo for halo in snap63_halos}
    ...
    if large_changes:
        ...
        for i, change_info in enumerate(large_changes[:10]):
            ...
            print(f"{YELLOW}\n    {i+1}. HaloIndex {change_info['HaloIndex']}:{NC}")
...
def main():
    ...
    print(f"{BLUE}{'=' * 60}{NC}")
    print(f"{BLUE}Test Suite: HaloIndex Validation (test_halo_index.py){NC}")
    print(f"{BLUE}{'=' * 60}{NC}")
    print()
    print(f"Repository root: {REPO_ROOT}")
    print(f"Mimic executable: {MIMIC_EXE}")
    print()
    print("Testing HaloIndex design:")
    print("  - Every new galaxy gets a unique HaloIndex")
    print("  - HaloIndex persists across all snapshots")
    ...
    tests = [
        test_haloindex_uniqueness_snapshot62,
        test_haloindex_persistence,
        test_haloindex_uniqueness_snapshot63,
        test_haloindex_physical_evolution,
    ]
