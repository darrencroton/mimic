...
static void init_source_halo(struct Halo *halo, struct GalaxyData *galaxy, int type) {
...
int test_main_branch_deep_copy_and_reset(void) {
  ...
  TEST_ASSERT_DOUBLE_EQUAL(workspace[0].dT, 4.0, 1e-6, "dT should use source and descendant times");
...
int test_orphan_conversion_and_local_central(void) {
  ...
  TEST_ASSERT_DOUBLE_EQUAL(workspace[1].Mvir, 0.0, 1e-6, "Orphan Mvir should be zero");
...
int test_type2_preserved_without_orphan_retransition(void) {
  ...
  TEST_ASSERT_DOUBLE_EQUAL(workspace[1].Mvir, 75.0, 1e-6, "Type 2 source Mvir should be preserved");
