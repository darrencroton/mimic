...
int test_virial_velocity_calculation(void) {
  ...
  TEST_ASSERT_DOUBLE_EQUAL(vvir, vvir_expected, 1e-6, "Vvir should match sqrt(G*Mvir/Rvir)");

  printf("  Mvir = %.2f, Rvir = %.4f → Vvir = %.2f km/s\n", mvir, rvir, vvir);
  printf("  Formula check: sqrt(%.2f * %.2f / %.4f) = %.2f km/s\n", MimicConfig.G, mvir, rvir,
         vvir_expected);
...
int test_virial_consistency_relations(void) {
  ...
  TEST_ASSERT_DOUBLE_EQUAL(vvir_sq, expected_vvir_sq, 1e-3, "Vvir^2 should equal G*Mvir/Rvir");
...
int test_virial_edge_cases(void) {
  ...
  TEST_ASSERT_DOUBLE_EQUAL(vvir_zero, 0.0, 1e-3, "Zero mass should give ~zero velocity");
