...
def assert_hdf5_schema_layout(output_file, expected_format_version):
    """
    ...
    Args:
        output_file (Path): HDF5 file to validate.
        expected_format_version (str): the hdf5_format_version this particular
            input carries. Required rather than defaulted: fresh output from the
            current build and the tracked baselines under tests/data/ happen to
            agree today, but that is a fact about those files, not a rule that
            baselines track the current build. A default would let a call site
            inherit a version it never actually checked.
    """
    try:
        import h5py
    except ImportError:
