...
def assert_hdf5_schema_layout(output_file, expected_format_version="1.2"):
    ...
    from the current build is "1.2", and the tracked baselines under
    tests/data/ were regenerated by the Spin units-label change, so they
    also carry "1.2" today. That is a historical fact about this baseline,
    not a rule that baselines track the current build automatically. Every
    call site should state the version it expects rather than rely on this
    default.
    """
    try:
        import h5py
