...
- **Run-end memory profile (added 2026-08-19): peak process RSS plus three measured terms** — `C`, the output buffer's realised capacity, and `G`, the galaxy pool's allocation high-water (with resident slot count and chunk slack), which are the two the snapshot driver's footprint is parametric in; plus `P`, the largest output population any one live buffer reached, which is what `MAX_HALO_ARRAY_SIZE` must be checked against. **`C` and `P` are different measurements and must not be conflated** — on the default pair they diverge ~5×, because `C` carries the `MAXHALOFAC` seed. `P` is scoped to whatever buffer the driver keeps live: one snapshot under the snapshot-ordered driver, one tree under the tree-ordered driver. Reported by `print_run_memory_profile()` (`src/util/run_profile.h`) from rank 0 at run end, **unconditionally — neither gated on `--verbose` nor suppressed by `--quiet`** (it saves, lowers and restores the log threshold around the block for exactly that reason), because a long run whose peak went unrecorded cannot be re-measured without repeating it. **Unit trap: this block reports GB = 1e9 B, while the allocator's own reports use MB = 1024²** — convert before comparing them. A term reads `unmeasured` rather than `0` when nothing sized that structure during the run. Peak RSS comes from `getrusage` (`ru_maxrss`: bytes on macOS, kilobytes on Linux — both branches verified against an independent measurement, Linux on RHEL 9 as of 2026-08-20) and is the number to trust: the tracked allocator's high-water cannot see a `realloc` that has to move its block, since `myrealloc_cat` subtracts the old allocation before calling libc.
- In-code instruments: `print_allocated()` / `print_allocated_by_category()` (`src/util/memory.h`) for point-in-time snapshots; `galaxy_pool_stats()` (`src/core/galaxy_pool.h`) for one pool's high-water, resident slots and chunk count.
