...
- **Run-end memory profile (added 2026-08-19): peak process RSS plus the two terms the snapshot driver's footprint is parametric in** — `C`, the output buffer's realised capacity, and `G`, the galaxy pool's allocation high-water (with resident slot count and chunk slack). Reported by `print_run_memory_profile()` (`src/util/run_profile.h`) from rank 0 at run end, **unconditionally — neither gated on `--verbose` nor suppressed by `--quiet`** (it saves, lowers and restores the log threshold around the block for exactly that reason), because a long run whose peak went unrecorded cannot be re-measured without repeating it. **Unit trap: this block reports GB = 1e9 B, while the allocator's own reports use MB = 1024²** — convert before comparing them. A term reads `unmeasured` rather than `0` when nothing sized that structure during the run. Peak RSS comes from `getrusage` (`ru_maxrss`: bytes on macOS, kilobytes on Linux) and is the number to trust: the tracked allocator's high-water cannot see a `realloc` that has to move its block, since `myrealloc_cat` subtracts the old allocation before calling libc.
