...
- **(g) Some "stale" generated files are not drift.** `init_halo_properties.inc`, `init_galaxy_properties.inc`, `reset_galaxy_properties.inc`, and `tests/generated/module_sources.mk` exist on disk but NO current generator writes them (legacy leftovers; the Makefile still names two). `make check-generated` passing while these look old is NOT a bug.
