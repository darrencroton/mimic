...
- **(i) The ctrees_ascii final-snapshot topology difference is a real defect, under active removal.** `fix_flybys` collapses every z=0 FoF group into one (negated `MostBoundID`); the lhalo/HDF5 readers do not. If a z=0 diagnostic looks wrong on `*-ascii` or `*-snapshot` data — baryon fraction above cosmic, a truncated Type-0 halo mass function, absurd halo occupation — this is the first thing to check. `docs/dev/SHIN-UCHUU-FLYBY-DEFECT-ADDENDUM.md` owns the fix.
