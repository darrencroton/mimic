...
- **(g) Some "stale" generated files are not drift.** `reset_galaxy_properties.inc` and `tests/generated/module_sources.mk` exist on disk but NO current generator writes them (legacy leftovers; the Makefile names neither). `make check-generated` passing while these look old is NOT a bug. `init_halo_properties.inc` and `init_galaxy_properties.inc` were the same class and were removed from `GENERATED_HEADERS` and archived on 2026-08-13.
