...
| `simulation.unique_galaxy_id_multiplier` | int64 > 0 | Optional. Forest multiplier in `UniqueGalaxyID = halonr + multiplier × (forestnr_global + 1)`. **Default: `TREE_MUL_FAC`** (10⁹, `src/include/constants.h`), seeded once before either parser pass, so the parser assigns only when the key is present — a `simulation_info.yaml` value survives a run file that omits it, and a run-file value wins. Zero or negative is fatal at parse time. **Both processing orders honour a non-default value:** every helper in `src/include/galaxy_id.h` takes the multiplier as an explicit `int64_t` parameter, and the three Consistent-Trees forest-size guards check against the same configured value, so a vertical configuration may set one. Every HDF5 run records the value as the `int64` `RunProperties/UniqueGalaxyIDMultiplier` attribute, in per-file outputs and in the master file. Horizontal readers additionally bounds-check it against the dataset headers at `open_run` (`horizontal_identity_bounds_valid`, which derives its bound from the same `galaxy_id.h` helper) |
...
| `tree_name` | string | Required unconditionally, but its **meaning is reader-specific** — it is not a general filename pattern. `lhalo_binary`: filename base before the file-number suffix, with the reader's extension appended. Both ctrees readers: a literal filename under `simulation_dir`. `lhalo_hdf5`: an explicit filename, optionally with a `%d` file-number placeholder. `horizontal_hdf5`: a *declaration of the format's fixed convention* — accepted only as the exact literal `snapshot_%03d.h5` (`HORIZONTAL_READER_TREE_NAME`, `src/io/horizontal/reader.h`), every other value rejected by `validate_and_postprocess` with a message naming the literal. The reader builds paths from a fixed internal format string; configured text is never used as a `printf` format |
| `tree_type` | string | Required. Resolved at parse time against **two** registries: `vertical_reader_lookup()` (`src/io/vertical/registry.c`) first, then `horizontal_reader_lookup()` (`src/io/horizontal/registry.c`). The name sets are disjoint, so the order fixes only which registry answers first. Unknown → FATAL naming both registries and reminding that HDF5 types need an HDF5-enabled build. Exactly one of `MimicConfig.vertical_reader` / `MimicConfig.horizontal_reader` is left non-NULL; `TreeExtension` is set only for vertical readers. Registered names: forest-ordered `lhalo_binary`, `lhalo_hdf5`, `consistent_trees_ascii`, `consistent_trees_hdf5`; horizontal `horizontal_hdf5` |
| `processing_order` | string | Case-insensitive `vertical` (default) or `horizontal`; anything else fatal. Validated against the resolved reader's own declared `processing_order`, whichever registry answered it, so a mismatched reader/order pair is a config error. A correctly paired `horizontal` configuration passes configuration validation (including the three horizontal-only rejections below the table) and reaches the live horizontal driver, `run_horizontal_driver()` (`src/core/horizontal_driver.c`), which calls the reader's `open_run` before processing anything, so the dataset is fully validated and a bad file aborts naming the file, object, and value |
...
These are not separate `input:` keys — they are additional config-time checks that apply only when the resolved `processing_order` is `horizontal`, each accumulated into the same "Parameter validation failed" report as every other required-key violation:
