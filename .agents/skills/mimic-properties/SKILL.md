...
Mimic runs in one fixed internal basis declared under `reference_units` in `src/core/core_properties.yaml`: mass `1e10 Msun/h`, length `Mpc/h`, velocity `km/s`, time derived (length/velocity), all `carried` except velocity (`none`). `reference_units` also declares derived (composite) dimensions available for property declarations (currently `specific_angular_momentum`, `Mpc/h km/s`); these are not additional basis elements — mass/length/velocity/time remains the complete dimensional basis. Every incoming quantity declares `units` (+ optional `h_convention`) and is converted at the boundary by generated code (`src/include/generated/unit_registry.h` for catalog fields; `parameter_unit_conversions.h` + `*_INTERNAL` macros for parameters declared in `models/<MODEL>/parameter_units.yaml` — name/type[`double` only]/units/h_convention; sham has one, sage16 none; unlisted parameters are assumed already in reference units).

Registered unit labels (the ONLY valid `units:` values; generation fails loudly on unknown labels): `dimensionless`, `index`, `identifier`, `count`, `particles`, `Internal`, `dex`, `Msun`, `Msun/h`, `1e10 Msun`, `1e10 Msun/h`, `cm`, `kpc`, `kpc/h`, `Mpc`, `Mpc/h`, `cm/s`, `km/s`, `s`, `yr`, `Myr`, `Myr/h`, `Gyr`, `Gyr/h`, `erg`, `erg/s`, `log10(erg/s)`, `Msun/yr`, `erg cm^3/s`, `Mpc/h km/s`. To add a label, extend `UNIT_REGISTRY` in `scripts/generate_properties.py` with its dimension, cgs magnitude, and default h_convention. Conversion rule: linear cgs scale plus a `Hubble_h` factor when source and target are both h-dependent but differ between `carried` and `free`; `none` cannot convert to/from h-dependent conventions. Time-unit conversions are deliberately rejected (reference time is derived).
...
**Both `src/include/generated/` and `tests/generated/` are gitignored, so `git diff` and `git status` can never verify them** — they show nothing whether generation is correct, stale, or absent. Use `make check-generated` (without `TEST_BUILD=yes`, which hashes a different input set) and read the files directly. Note the provenance hash lands in three shapes, not one: the `Source MD5:` comment header, a runtime JSON value inside `output_schema_writer.inc` that reaches every run's `metadata/output_schema.json`, and headerless `tests/generated/property_ranges.json`.

Known quirk: `tests/generated/module_sources.mk` exists on disk but is a **stale legacy artifact no current generator writes** (init/reset logic moved into `property_defs.h` inline functions). Don't chase it as drift and don't hand-edit it. The Makefile's `GENERATED_HEADERS` **no longer names any of that class**: `init_halo_properties.inc` and `init_galaxy_properties.inc` were verified orphaned on both ends (no generator writes them, no C source includes them), removed from `GENERATED_HEADERS`, and moved to `archive/orphaned-generated/` on 2026-08-13; `reset_galaxy_properties.inc` was verified the same way and archived there on 2026-08-14. Because `src/include/generated/` is gitignored, an older worktree may still carry any of them.
