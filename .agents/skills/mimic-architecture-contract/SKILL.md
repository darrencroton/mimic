...
| Galaxy pool is bulk-reset once per processing unit — once per tree for the vertical driver; once per snapshot, ping-ponged across two instances, for the horizontal driver | Instanced `struct GalaxyPool *` API (`src/core/galaxy_pool.h`) | Per-galaxy frees would be slow and leak-prone; nothing may hold pool pointers across units |
...
- **Reader** (`input.tree_type`): how trees are stored on disk. Forest-ordered formats sit behind the `struct VerticalReader` vtable in `src/io/vertical/reader.h` (12 function-pointer hooks), registered in `src/io/vertical/registry.c` (`lhalo_binary`, `lhalo_hdf5`, `consistent_trees_ascii`, `consistent_trees_hdf5`). Each declares a partition model: `PARTITION_PER_FILE` (work unit = input file) or `PARTITION_ENUMERATED` (reader enumerates forests as work units).
- **Horizontal readers are a second family, behind a second registry.** `struct HorizontalReader` (`src/io/horizontal/reader.h`) is a separate small vtable — `open_run`, `close_run`, `snapshot_halo_count`, `load_slab`, `release_slab` — registered in `src/io/horizontal/registry.c` (`horizontal_hdf5`). It is deliberately NOT a widening of `struct VerticalReader`: the tree hooks are partition/unit-shaped, and two disjoint hook sets in one struct would defeat the `REQUIRE_READER_HOOK` fail-fast. Horizontal readers have no partitions and no units; the working set is one snapshot's slab, with `int64_t` counts throughout.
...
- **W1 — CLOSED 2026-08-12 (dual-driver Phase 5).** `horizontal` now has a live driver: `run_horizontal_driver()` (`src/core/horizontal_driver.c`), dispatched from `run_processing_driver()`'s `INPUT_PROCESSING_ORDER_HORIZONTAL` case. It calls `horizontal_reader_open_run()` before processing anything, so a horizontal run now proves its dataset readable — structure, headers, `scale_factor` agreement, physical-header agreement with configuration, and measured identity bounds — before any halo is processed, and `load_slab`'s link-range validation runs once per snapshot as the driver loads each slab. The HDF5 output writers no longer dereference `MimicConfig.vertical_reader` at all (see the input-view/output-partition-seam paragraph above); both were made reader-kind-neutral as part of this closure. See `docs/DEVELOPER-GUIDE.md` → "The Horizontal Driver" for the loop, the two-generation rotation, and the parity checklist the cross-format identity gate verifies.
...
Written 2026-07-04 against Mimic v1.0 (tagged 2026-06-29); the reader/driver seam and weak points W1–W2 updated 2026-08-04 when the horizontal reader landed, and updated again 2026-08-12 when the horizontal driver landed (dual-driver Phase 5) and both W1 and W2 closed. The principles and invariants are durable; the function names and weak points can drift. Re-verify before relying on specifics:
