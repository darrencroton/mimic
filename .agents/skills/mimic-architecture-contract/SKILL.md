...
- **Driver** (`input.processing_order`): the order halos are processed. `tree_ordered` is the shipped driver (`run_processing_driver` in src/core/tree_driver.c); `snapshot_ordered` resolves and clears *configuration* validation but has no driver (weak point W1 — read it before assuming anything about the input having been checked). Startup validation compares the resolved reader's declared `processing_order` against the configured one, whichever registry answered.
