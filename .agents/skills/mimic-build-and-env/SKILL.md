...
- The Makefile resolves tools itself, and **activation is not required** (since 2026-09-22): `PYTHON` prefers an activated `$VIRTUAL_ENV`, then `mimic_venv/bin/python3`, then system `python3`, and every generator/validator target uses it. `CLANG_FORMAT` uses `mimic_venv/bin/clang-format` whenever the file exists. `beautify.sh` prefers the venv's clang-format, black and isort, and exits non-zero if a requested formatter is missing. Shell scripts resolve the same way by sourcing `scripts/lib/python.sh`, which exports `MIMIC_PYTHON` (`tests/unit/run_tests.sh`, `tests/unit/tools/build_topology_dump.sh`, `scripts/regenerate_baseline.sh`, `scripts/benchmark_mimic.sh`); `scripts/first_run.sh` deliberately still uses system `python3`, since it is what creates the venv. Before that fix, `PYTHON` consulted the venv only when `$VIRTUAL_ENV` already named it — which is exactly when bare `python3` resolves there anyway — and 13 Makefile targets plus 12 shell call sites hardcoded `python3`, so `make generate`, a plain `make`, `lint-parameters`, `check-format` and the whole `tests-unit` tier failed from an unactivated shell with "PyYAML not installed" or "No module named black".
