...
static inline int64_t mimic_unique_galaxy_id_max_forests(void) {
  return (LLONG_MAX - (TREE_MUL_FAC - 1LL)) / TREE_MUL_FAC;
}
...
static inline int64_t mimic_encode_unique_galaxy_id(int64_t halonr, int64_t forestnr_global) {
  return halonr + TREE_MUL_FAC * (forestnr_global + 1LL);
}
