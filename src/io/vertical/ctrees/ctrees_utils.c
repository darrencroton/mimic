/**
 * @file    vertical/ctrees/ctrees_utils.c
 * @brief   Format-independent Consistent-Trees topology reconstruction.
 *
 * Ported from sage-model (io/ctrees_utils.c) with minimal edits, see the header
 * for the design rationale and the ctrees_compat.h seam. Edits relative to
 * upstream:
 *   - includes point at Mimic headers; getnumlines() is vendored here as a
 *     file-local static (sage keeps it in core_utils.c);
 *   - malloc/calloc/realloc/free become Mimic's categorised allocator
 *     (MEM_IO for setup scaffolding, MEM_TREES for per-forest topology scratch);
 *     the upstream allocation NULL-checks are dropped because mymalloc/myrealloc
 *     abort on failure rather than returning NULL;
 *   - file-local helpers are marked static, and four genuinely-unused upstream
 *     sort helpers (sort_locations_file_offset / sort_locations_on_fid and their
 *     comparators) are omitted rather than vendored as dead code.
 */
...
static int64_t getnumlines(const char *fname, const char comment);
static int64_t find_fof_halo(const int64_t totnhalos, const struct additional_info *info,
                             const int start_loc, const int64_t upid, int verbose,
                             int64_t calldepth);

/* Vendored from sage-model core_utils.c: count the non-blank, non-comment lines
   in a text file (used to size the forest/location index arrays up front). */
static int64_t getnumlines(const char *fname, const char comment) {
  enum { MAXLINESIZE = 10000 }; /* integer constant expression -> not a VLA */
  int64_t nlines = 0;
  ...
  while (1) {
    if (fgets(str_line, MAXLINESIZE, fp) != NULL) {
      /* fgets always terminates the string with a '\0' on a successful read */
      char *c = &str_line[0];
      while (*c != '\0' && isspace(*c)) {
        c++;
      }
...
int64_t read_forests(const char *filename, int64_t **f, int64_t **t) {
  ...
  /* By passing the comment character, getnumlines will return the actual number
  ...
  *f = mymalloc_cat(ntrees * sizeof(int64_t), MEM_IO);
  *t = mymalloc_cat(ntrees * sizeof(int64_t), MEM_IO);
...
int64_t read_locations(const char *filename, const int64_t ntrees, struct locations_with_forests *l,
  ...
  files_fd->fd = mymalloc_cat(numfiles_allocated * sizeof(files_fd->fd[0]), MEM_IO);
  files_fd->nallocated = numfiles_allocated;
  files_fd->numfiles = 0;

  files_fd->numtrees_per_file =
      mymalloc_cat(numfiles_allocated * sizeof(files_fd->numtrees_per_file[0]), MEM_IO);
  memset(files_fd->numtrees_per_file, 0,
         numfiles_allocated * sizeof(files_fd->numtrees_per_file[0]));
  ...
  while (fgets(buffer, MAX_STRING_LEN, fp) != NULL) {
    if (buffer[0] == comment) {
      ...
      while (fileid >= files_fd->nallocated) {
        ...
        void *new_numtrees =
            myrealloc_cat(files_fd->numtrees_per_file,
                          numfiles_allocated * sizeof(files_fd->numtrees_per_file[0]), MEM_IO);
  ...
  XRETURN(max_fileid + 1 == files_fd->numfiles, -EXIT_FAILURE,
          "Error: Validation error -- number of files expected from max. of fileids in "
          "'locations.dat' = %" PRId64 " but only found %d filenames\n"
...
#define MULTIPLE_ARRAY_EXCHANGER(type, a, i, j)                                                    \
  ...
  SGLIB_ARRAY_HEAP_SORT(int64_t, treeids, ntrees, SGLIB_NUMERIC_COMPARATOR,
...
static void sort_locations_on_treeroot(const int64_t ntrees,
...
int assign_forest_ids(const int64_t ntrees, struct locations_with_forests *locations,
                      int64_t *forests, int64_t *treeids) {
  ...
  for (int64_t i = 0; i < ntrees; i++) {
    XRETURN(treeids[i] == locations[i].treeid, -EXIT_FAILURE,
            "tree roots[%" PRId64 "] = %" PRId64
            " does not equal tree roots in locations = %" PRId64 "\n",
...
#define MULTIPLE_ARRAY_EXCHANGER(type, a, i, j)                                                    \
  ...
  for (int64_t i = 0; i < totnhalos; i++) {
    ...
    if (verbose) {
      fprintf(stderr,
              "CALLING FIND FOF HALO with i = %" PRId64 " id = %" PRId64 " upid = %" PRId64 "\n", i,
              info[i].id, upid);
    }
    int64_t loc = find_fof_halo(totnhalos, info, i, upid, verbose, calldepth);
    while (loc < 0 || loc >= totnhalos) {
      fprintf(stderr,
              "looping to locate fof halo for i = %" PRId64 " id = %" PRId64 " upid = %" PRId64
              " loc=%" PRId64 "\n",
    ...
    if (verbose) {
      fprintf(stderr,
              "found FOF halo for halnum = %" PRId64 ". loc = %" PRId64 " id = %" PRId64
              " upid = %" PRId64 "\n",
              i, loc, info[loc].id, info[loc].upid);
    }
    XRETURN(loc >= 0 && loc < totnhalos, -EXIT_FAILURE,
            "could not locate fof halo for i = %" PRId64 " id = %" PRId64 " upid = %" PRId64
            " loc=%" PRId64 "\n",
            i, info[i].id, upid, loc);
    const int64_t new_upid = info[loc].id;
    if (verbose) {
      fprintf(stderr,
              "setting upid/pid for halonum = %" PRId64 " to %" PRId64
              ". previously: pid = %" PRId64 " upid = %" PRId64 ". id = %" PRId64 "\n",
...
int assign_mergertree_indices(const int64_t totnhalos, struct halo_data *forest,
  ...
  double *scales = mymalloc_cat(nsnapshots * sizeof(*scales), MEM_TREES);
  for (int i = 0; i < nsnapshots; i++) {
    scales[i] = DBL_MAX;
  }
  int64_t *start_scale = mymalloc_cat(nsnapshots * sizeof(*start_scale), MEM_TREES);
  int64_t *end_scale = mymalloc_cat(nsnapshots * sizeof(*end_scale), MEM_TREES);
  for (int i = 0; i < nsnapshots; i++) {
    start_scale[i] = -1;
  }
...
#define UPID_COMPARATOR(x, y)                                                                      \
  ((x.upid > y.upid ? 1 : (x.upid < y.upid ? -1 : PID_COMPARATOR(x, y))))
/* Note, the negated order in the scale comparison . this ensures descending sort */
#define SCALE_UPID_COMPARATOR(x, y)                                                                \
  ((x.scale > y.scale ? -1 : (x.scale < y.scale ? 1 : UPID_COMPARATOR(x, y))))
#define MULTIPLE_ARRAY_EXCHANGER(type, a, i, j)                                                    \
  {                                                                                                \
    SGLIB_ARRAY_ELEMENTS_EXCHANGER(struct halo_data, forest, i, j);                                \
    SGLIB_ARRAY_ELEMENTS_EXCHANGER(struct additional_info, info, i, j)                             \
  ...
  /* I am using heap sort rather than qsort because the forest is already somewhat sorted. For
     sorted data, qsort behaves closer to O(N^2).
   */
  SGLIB_ARRAY_HEAP_SORT(struct additional_info, info, totnhalos, SCALE_UPID_COMPARATOR,
                        MULTIPLE_ARRAY_EXCHANGER);
  ...
  for (int64_t i = 0; i < totnhalos; i++) {
    ...
    if (info[i].pid == -1) {
      XRETURN(i < INT_MAX, EXIT_FAILURE, "Assigning to integer i = %" PRId64 " is more than %d\n",
              i, INT_MAX);
      ...
      if (FirstHaloInFOFgroup == -1) {
        ...
        for (int64_t k = 0; k < totnhalos; k++) {
          fprintf(stderr, "%03d %12.5lf %10" PRId64 " %10" PRId64 " %10" PRId64 " %12.4g\n",
                  forest[k].SnapNum, info[k].scale, info[k].upid, info[k].pid, info[k].id,
                  forest[k].Mvir);
        }
      }
      ...
      if (info[i].upid == fof_id) {
        ...
        /* Should not reach here..I have already sorted the forest such that the FOF appears before
         * the subs */
        fprintf(
            stderr,
            "ERROR: the sort did not place the FOF before the subs. BUG IN CTREES OR IN SORT\n");
        for (int64_t k = 0; k < totnhalos; k++) {
          fprintf(stderr, "%03d %12.5lf %10" PRId64 " %10" PRId64 " %10" PRId64 " %12.4g\n",
                  forest[k].SnapNum, info[k].scale, info[k].upid, info[k].pid, info[k].id,
                  forest[k].Mvir);
        ...
        fprintf(stderr,
                "i = %" PRId64 " id = %" PRId64 " pid = %" PRId64 " fof_id = %" PRId64
                " upid = %" PRId64 " FirstHaloInFOFgroup = %" PRId64 "\n",
      ...
      while (forest[insertion_point].NextHaloInFOFgroup != -1) {
        ...
        XRETURN(nexthalo >= 0 && nexthalo < totnhalos, EXIT_FAILURE,
                "Inserting next halo in FOF group into invalid index. nexthalo = %d totnhalos = "
                "%" PRId64 "\n",
                nexthalo, totnhalos);
  ...
  /* Now figure out merger tree pointers. Need to set descendant, firstprogenitor and
   * nextprogenitor.
   */
  for (int64_t i = 0; i < totnhalos; i++) {
    if (info[i].descid == -1) {
    ...
    while ((desc_snapnum >= 0) && (fabs(scales[desc_snapnum] - desc_scale) > max_epsilon_scale)) {
      desc_snapnum--;
    }
    XRETURN(desc_snapnum >= 0 && desc_snapnum < nsnapshots &&
                (fabs(scales[desc_snapnum] - desc_scale) <= 1e-4),
            EXIT_FAILURE, "Could not locate desc_snapnum. desc_snapnum = %d nsnapshots = %d \n",
            desc_snapnum, nsnapshots);

    /*start_scale and end_scale are inclusive. Hence the stopping condition is "<=" rather than
     * simply "<" */
    int64_t desc_loc = start_scale[desc_snapnum];
    while (desc_loc >= start_scale[desc_snapnum] && desc_loc <= end_scale[desc_snapnum] &&
           info[desc_loc].id != descid) {
      desc_loc++;
    }
    ...
    XRETURN(info[desc_loc].id == descid, EXIT_FAILURE,
            "Should have found descendant id = %" PRId64 " but info[%" PRId64 "]=%" PRId64
            " instead \n",
            descid, desc_loc, info[desc_loc].id);

    XRETURN(desc_loc < INT_MAX, EXIT_FAILURE,
    ...
    if (forest[desc_loc].FirstProgenitor == -1) {
      ...
      */
      ...
      XRETURN(first_prog >= 0 && first_prog < totnhalos, EXIT_FAILURE,
              "first_prog=%d must lie within [0, %" PRId64 "\n", first_prog, totnhalos);
      if (forest[first_prog].Mvir < forest[i].Mvir) {
        XRETURN(i < INT_MAX, EXIT_FAILURE,
                "Assigning Nextprogenitor = %" PRId64
                " to an int will result in garbage. INT_MAX = "
                "%d\n",
                i, INT_MAX);
        forest[desc_loc].FirstProgenitor = i;
        ...
        while (forest[insertion_point].NextProgenitor != -1) {
          ...
          XRETURN(next_prog >= 0 && next_prog < totnhalos, EXIT_FAILURE,
                  "Inserting next progenitor into invalid index. insertion_point = %" PRId64
                  " totnhalos = %" PRId64 "\n",
                  next_prog, totnhalos);
...
static int64_t find_fof_halo(const int64_t totnhalos, const struct additional_info *info,
  ...
  XRETURN(calldepth <= max_recursion_depth, -EXIT_FAILURE,
          "%s has been recursively called %" PRId64
          " times already. Likely caught in infinite loop..exiting\n",
  ...
  while (start_loc >= 0 && start_loc < totnhalos && info[start_loc].pid != -1) {
    if (verbose == 1) {
      fprintf(stderr, "start_loc = %d id = %" PRId64 " pid = %" PRId64 "\n", start_loc,
              info[start_loc].id, info[start_loc].pid);
      fprintf(stderr, "scale = %lf pid = %" PRId64 " upid = %" PRId64 "\n", info[start_loc].scale,
              info[start_loc].pid, info[start_loc].upid);
    }
    ...
    if (info[loc].pid == -1) {
      ...
      if (verbose == 1) {
        fprintf(stderr,
                "calling find_fof_halo again with loc =%" PRId64 " (int) loc = %d start_loc was "
                "=%d\n",
                loc, (int)loc, start_loc);
        fprintf(stderr,
                "scale = %lf id = %" PRId64 " pid = %" PRId64 " upid = %" PRId64
                " calldepth=%" PRId64 "\n",
