/**
 * @file    vertical/ctrees/forest_utils.c
...
int distribute_forests_over_ntasks(const int64_t totnforests, const int NTasks, const int ThisTask,
  ...
  if (ThisTask < rem_nforests) {
    ...
    start_forestnum +=
        rem_nforests; // All tasks that weren't given an extra forest will be offset by a constant.
  }
...
double compute_forest_cost_from_nhalos(const enum ForestDistributionScheme forest_weighting,
  ...
  case uniform_in_forests:
    /* Every forest is treated equally -> every forest contributes one unit of compute cost */
    cost = 1.0;
    break;
  ...
  case exponent_in_nhalos: {
    if (exponent < 0) {
      fprintf(stderr,
              "Error: The power-law index = %e (should be integer) for computing the weights of "
              "individual forests must be > 0\n",
              exponent);
      return -1;
    }
  ...
  case generic_power_in_nhalos:
    if (exponent < 0) {
      fprintf(
          stderr,
          "Error: The power-law index = %e for computing the weights of individual forests must "
          "be > 0\n",
          exponent);
      return -1;
    }
...
int distribute_weighted_forests_over_ntasks(const int64_t totnforests,
  ...
  if (totnforests == 0 || NTasks == 1) {
    *nforests_thistask =
        totnforests; /* totnforests could be 0, or all the forests if running on a single task*/
    *start_forestnum_thistask = 0;
    return EXIT_SUCCESS;
  }

  if (forest_weighting == uniform_in_forests || nhalos_per_forest == NULL) {
    return distribute_forests_over_ntasks(totnforests, NTasks, ThisTask, nforests_thistask,
                                          start_forestnum_thistask);
  }

  if ((forest_weighting == exponent_in_nhalos || forest_weighting == generic_power_in_nhalos) &&
      power_law_index < 0) {
    fprintf(stderr,
            "Error: You have requested a power-law exponent but the exponent = %e must be greater "
            "than 0\n",
            power_law_index);
    return EXIT_FAILURE;
  }
  ...
  for (int64_t i = 0; i < totnforests; i++) {
    const double cost_this_forest =
        compute_forest_cost_from_nhalos(forest_weighting, nhalos_per_forest[i], power_law_index);
  ...
  for (int64_t i = 0; i < totnforests; i++) {
    ...
    if (cost_so_far < curr_cost_target)
      continue;
...
int find_start_and_end_filenum(const int64_t start_forestnum, const int64_t end_forestnum,
  ...
  if (start_forestnum >= end_forestnum) {
    fprintf(stderr,
            "Error (line %d in function '%s'): Expected that the starting forest number = %" PRId64
            " "
  ...
  for (int filenr = firstfile; filenr <= lastfile; filenr++) {
    ...
    if (start_forestnum >= nforests_so_far && start_forestnum < end_forestnum_this_file) {
      ...
      num_forests_to_process_per_file[filenr] =
          nforests_this_file - (start_forestnum - nforests_so_far);
    }

    if (end_forestnum >= nforests_so_far && end_forestnum <= end_forestnum_this_file) {
      num_forests_to_process_per_file[filenr] =
          end_forestnum - (start_forestnum_to_process_per_file[filenr] + nforests_so_far);
      end_filenum = filenr;

      XRETURN(num_forests_to_process_per_file[filenr] <= nforests_this_task, -1,
              "Error: num_forests_to_process_per_file[%d] = %" PRId64 " start_filenum = %d "
              "end_filenum = %d "
              "start_forestnum = %" PRId64 " end_forestnum = %" PRId64 " nforests_this_task = "
              "%" PRId64 " nforests_so_far = %" PRId64 "\n",
              filenr, num_forests_to_process_per_file[filenr], start_filenum, end_filenum,
              start_forestnum, end_forestnum, nforests_this_task, nforests_so_far);
      break;
    }
  ...
  if (start_filenum == -1 || end_filenum == -1) {
    fprintf(stderr,
            "Error: Could not locate start (=%d) or end file number (=%d) for processing forests\n",
            start_filenum, end_filenum);
    fprintf(stderr, "Printing debug info\n");
    fprintf(stderr,
            "ThisTask = %d NTasks = %d totnforests = %" PRId64 " start_forestnum = %" PRId64
            " end_forestnum = %" PRId64 "\n",
            ThisTask, NTasks, totnforests, start_forestnum, end_forestnum);
    for (int filenr = firstfile; filenr <= lastfile; filenr++) {
      fprintf(stderr, "filenr := %d contains %" PRId64 " forests ", filenr,
