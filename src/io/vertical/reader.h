#ifndef IO_VERTICAL_READER_H
#define IO_VERTICAL_READER_H

#include <stddef.h>
#include <stdint.h>

/**
 * @file    vertical/reader.h
 * @brief   Format-agnostic merger-tree reader interface.
 *
 * Each supported input format registers exactly one VerticalReader (see
 ...
 * within a partition.
 *
 * Two partition models are supported (see the driver loop in core/vertical_driver.c):
 *
...
enum InputProcessingOrder {
  INPUT_PROCESSING_ORDER_VERTICAL = 0,
  INPUT_PROCESSING_ORDER_HORIZONTAL = 1,
};
...
struct VerticalReader {
  ...
  /* Processing-order driver this reader feeds. Current readers are vertical. */
 ...
 * id equal to the file number. Defined in vertical/interface.c.
 */
