 ...
 *
 * Dispatches to the format-specific reader (MimicConfig.vertical_reader, see vertical/registry.c),
 * then allocates and zeroes per-snapshot halo counters (InputHalosPerSnap, TotHalosPerSnap).
 */
void open_partition(int output_id) {
  int i, n;

  MimicConfig.vertical_reader->open_partition(output_id);
...
static void format_tree_partition_path(const struct VerticalReader *reader, char *tree_path,
...
int tree_partition_per_file_exists(int partition) {
  ...
  format_tree_partition_path(MimicConfig.vertical_reader, tree_path, sizeof(tree_path), output_id);
...
void load_unit(int unit) {
  ...
  MimicConfig.vertical_reader->load_unit(unit);
