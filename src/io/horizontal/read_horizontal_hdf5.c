 ...
 *
 * Reads the frozen horizontal HDF5 contract described in
 * docs/dev/HORIZONTAL-HDF5-FORMAT.md (format_version = 2): one
...
/* Horizontal-flavoured counterparts of the vertical reader's macros
   ...
   to the field's type -- the vertical reader's cast puns a double * as an int * or
   float *, which is an aliasing violation cppcheck flags. */
#define READ_TREE_PROPERTY(field_name, hdf5_name, type_int, data_type)                             \
  {                                                                                                \
    horizontal_h5_read_column(file, path, hdf5_name, HORIZONTAL_H5_MEMTYPE(type_int), n_halos, 1,  \
                              buffer);                                                             \
...
#define READ_TREE_PROPERTY_MULTIPLEDIM(field_name, hdf5_name, type_int, data_type)                 \
  {                                                                                                \
    horizontal_h5_read_column(file, path, hdf5_name, HORIZONTAL_H5_MEMTYPE(type_int), n_halos,     \
                              NDIM, buffer_multipledim);                                           \
    for (int64_t halo_idx = 0; halo_idx < n_halos; ++halo_idx) {                                   \
...
static void horizontal_h5_fill_halos(hid_t file, const char *path, int64_t n_halos,
  ...
  char *buffer = mymalloc_cat(HORIZONTAL_H5_MAX_ELEMENT_SIZE * (size_t)n_halos, MEM_TREES);
  char *buffer_multipledim =
      mymalloc_cat(HORIZONTAL_H5_MAX_ELEMENT_SIZE * (size_t)n_halos * NDIM, MEM_TREES);
