...
extern int sage_handle_mergers_immediate_process(struct ModuleContext *ctx,
                                                 struct Halo *halos, int ngal);
extern int sage_handle_mergers_immediate_cleanup(void);
extern void sage_handle_mergers_immediate_set_action_hook(
...
static int init_parity_modules(void) {
  if (sage_handle_mergers_immediate_init() != 0) {
    return -1;
  }

  sage_handle_mergers_immediate_set_action_hook(record_immediate_action);
  return 0;
}

static void cleanup_parity_modules(void) {
  sage_handle_mergers_immediate_set_action_hook(NULL);
...
int test_shared_target_immediate_trace(void) {
  ...
  TEST_ASSERT(sage_handle_mergers_immediate_process(&ctx, halos, 3) == 0,
...
int test_consumed_target_redirect_trace(void) {
  ...
  TEST_ASSERT(sage_handle_mergers_immediate_process(&ctx, halos, 4) == 0,
              "Immediate parity module should succeed");
  TEST_ASSERT(halos[1].Type == 3,
