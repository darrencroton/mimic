...
Test cases:
  ...
  - test_property_modification: EjectedGas and HotGas modification
...
def test_property_modification():
    """Test that module modifies EjectedGas and HotGas properties"""
    print(f"\n{BLUE}TEST:{NC} Property modification (EjectedGas → HotGas)")

    # Check if sage_calculate_infall is available (needed to populate EjectedGas)
    available = get_available_modules()
    if 'sage_calculate_infall' not in available:
        print(f"  {YELLOW}⚠ SKIP:{NC} sage_calculate_infall not available (needed to populate EjectedGas)")
        return True  # Skip, not a failure
    ...
    try:
        ...
        required_props = ['EjectedGas', 'HotGas', 'MetalsEjectedGas', 'MetalsHotGas']
