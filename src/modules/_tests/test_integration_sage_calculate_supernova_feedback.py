...
- SupernovaReheatedMass and SupernovaEjectedMass properties are calculated
...
def setup_module():
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="test_snfb_"))
...
def test_module_loads():
    ...
    param_file = create_test_config(
        ['sage_calculate_star_formation', 'sage_calculate_supernova_feedback'],
        output_dir
    )
...
def test_output_properties_exist():
    ...
    # Check that feedback fields exist
    assert hasattr(halos, 'SupernovaReheatedMass'), "Output should have SupernovaReheatedMass field"
    assert hasattr(halos, 'SupernovaEjectedMass'), "Output should have SupernovaEjectedMass field"
...
def test_parameters_configurable():
    ...
    config['modules']['enabled'] = ['sage_calculate_star_formation', 'sage_calculate_supernova_feedback']
    config['output']['output_dir'] = str(output_dir)
    ...
    if 'parameters' not in config:
        config['parameters'] = {}
    config['parameters']['FeedbackReheatingEpsilon'] = 5.0  # Custom value
    config['parameters']['FeedbackEjectionEfficiency'] = 0.8  # Custom value
...
def test_pipeline_with_star_formation():
    ...
    # Load output and validate both modules' outputs exist
    output_file = output_dir / "model_000.bin"
    halos = load_binary_halos(output_file)

    # Both SF and feedback properties should exist
    assert hasattr(halos, 'NewStellarMass'), "Should have NewStellarMass from SF module"
    assert hasattr(halos, 'SupernovaReheatedMass'), "Should have SupernovaReheatedMass from feedback module"
    assert hasattr(halos, 'SupernovaEjectedMass'), "Should have SupernovaEjectedMass from feedback module"
