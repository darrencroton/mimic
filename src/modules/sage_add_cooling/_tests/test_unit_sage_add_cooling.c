/**
 * @file    test_unit_sage_add_cooling.c
 * @brief   Software quality unit tests for sage_add_cooling module
 *
 * Validates: Module lifecycle, memory safety, error handling, cooling physics
 *
 * This test validates software engineering aspects of the sage_add_cooling module:
...
extern int sage_add_cooling_init(void);
extern int sage_add_cooling_process(struct ModuleContext *ctx, struct Halo *halos, int ngal);
extern int sage_add_cooling_cleanup(void);

/* Test fixtures for physics tests */

/**
 * @brief Initialize sage_add_cooling module for physics testing
 *
 ...
 * @brief   Test that sage_add_cooling module registers correctly
 *
 * Expected: Module registration succeeds without errors
 * Validates: sage_add_cooling_register() works, module appears in registry
 */
...
int test_module_initialization(void)
{
    ...
    /* Configure sage_add_cooling module in phase_1 */
    MimicConfig.phase_1 = mymalloc_cat(sizeof(struct PhaseModuleConfig), MEM_UTILITY);
    MimicConfig.phase_1[0].module_name = strdup("sage_add_cooling");
...
int test_physics_partial_cooling(void)
{
    ...
    /* ===== EXECUTE ===== */
    int result = sage_add_cooling_process(&ctx, &test_halo, 1);
...
int test_physics_metallicity_preservation(void)
{
    ...
    TEST_ASSERT_DOUBLE_EQUAL(z_hot, 0.05, 0.0001,
                             "Hot gas metallicity should be preserved at 0.05");
...
int test_zero_cooling(void)
{
    ...
    /* ===== VALIDATE ===== */
    TEST_ASSERT(result == 0, "Module processing should succeed");
    TEST_ASSERT_DOUBLE_EQUAL(test_halo.galaxy->HotGas, 10.0, 0.001,
                             "HotGas should be unchanged");
    TEST_ASSERT_DOUBLE_EQUAL(test_halo.galaxy->ColdGas, 5.0, 0.001,
...
int test_type2_orphan_skip(void)
{
    ...
    /* ===== EXECUTE ===== */
    int result = sage_add_cooling_process(&ctx, &orphan, 1);
...
int test_invalid_ngal(void)
{
    ...
    /* ===== EXECUTE ===== */
    int result = sage_add_cooling_process(&ctx, halos, 2);  /* ngal=2, should fail */
 ...
 *
 * Executes all sage_add_cooling software quality tests and reports results.
 */
int main(void)
{
    printf("%s", BLUE);
    printf("============================================================\n");
    printf("Test Suite: sage_add_cooling Module\n");
    printf("============================================================\n");
    printf("%s\n", NC);
