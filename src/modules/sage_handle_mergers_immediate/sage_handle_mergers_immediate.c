/**
 * @file    sage_handle_mergers_immediate.c
...
void sage_handle_mergers_immediate_set_action_hook(
...
int sage_handle_mergers_immediate_init(void) {
  ...
  LOAD_AND_VALIDATE_RANGE_EXCLUSIVE("ThresholdSatDisruption",
                                    THRESHOLD_SAT_DISRUPTION, 0.0, 1000.0,
                                    "halo-to-baryonic mass ratio threshold");

  INFO_LOG("SAGE immediate merger/disruption handler initialized");
  VERBOSE_LOG("  ThresholdMajorMerger = %.3f", THRESHOLD_MAJOR_MERGER);
  VERBOSE_LOG("  ThresholdSatDisruption = %.3f", THRESHOLD_SAT_DISRUPTION);
  return 0;
}

int sage_handle_mergers_immediate_cleanup(void) {
  action_hook = NULL;
  return 0;
}

int sage_handle_mergers_immediate_process(struct ModuleContext *ctx,
                                          struct Halo *halos, int ngal) {
  if (ctx == NULL || ctx->num_substeps <= 0) {
    ERROR_LOG("Invalid immediate merger context (ctx=%p, num_substeps=%d)",
  ...
  for (int i = 0; i < ngal; i++) {
    ...
    if (dt_status != MIMIC_OBJECT_TIME_OK) {
      ERROR_LOG("Invalid immediate-merger dt for halo %lld (SnapNum=%d, dT=%.3e, num_substeps=%d, status=%s)",
    ...
    if (satellite->MergTime >= 999.0) {
      ERROR_LOG("Satellite %lld has unset MergTime (%.1f)", halos[i].HaloNr,
                satellite->MergTime);
      return -1;
    }
    ...
    const double fraction = ((double)ctx->substep_number + 1.0) /
                            (double)ctx->num_substeps;
    double current_mvir =
        halos[i].Mvir - halos[i].deltaMvir * (1.0 - fraction);
    if (current_mvir < 0.0) {
      current_mvir = 0.0;
    }
    ...
    const int eligible =
        (galaxy_baryons == 0.0) ||
        (galaxy_baryons > 0.0 &&
         (virial_to_baryons <= THRESHOLD_SAT_DISRUPTION));
    ...
    const int target_idx =
        mimic_resolve_immediate_target_index(halos, ngal, i, fof_central_idx);
    if (target_idx < 0 || target_idx >= ngal || target_idx == i ||
        halos[target_idx].galaxy == NULL) {
      ERROR_LOG("Invalid immediate merger target (satellite=%d, target=%d)", i,
                target_idx);
      return -1;
    }
    ...
    if (time_status != MIMIC_OBJECT_TIME_OK) {
      ERROR_LOG("Invalid immediate-merger event time for halo %lld (status=%s)",
                halos[i].HaloNr, mimic_object_time_status_str(time_status));
      return -1;
    }
