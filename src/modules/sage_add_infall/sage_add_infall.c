 ...
 * @brief   SAGE add infall module - transfers infalling gas to hot reservoir
 *
 * Distributes infalling gas (from sage_calculate_infall) to hot reservoir over substeps
...
int sage_add_infall_process(struct ModuleContext *ctx, struct Halo *halos, int ngal)
{
    ...
    DEBUG_LOG("AddInfall: Mvir=%.3e, InfallingGas/step=%.3e, HotGas=%.3e, substep=%d/%d",
              halos[central_idx].Mvir, infallingGas, galaxy->HotGas,
