/**
 * @file    test_unit_sage_calculate_infall.c
 * @brief   Comprehensive unit tests for sage_calculate_infall module
 *
...
static void setup_module_for_physics_test(double global_baryon_frac)
{
    ...
    /* Configure sage_calculate_infall module in pre_timestep phase */
    MimicConfig.pre_timestep = mymalloc_cat(sizeof(struct PhaseModuleConfig), MEM_UTILITY);
    MimicConfig.pre_timestep[0].module_name = strdup("sage_calculate_infall");
 ...
 * Validates: sage_calculate_infall_register() works
 */
...
int test_memory_safety(void)
{
    ...
    /* ===== EXECUTE ===== */
    extern int sage_calculate_infall_process(struct ModuleContext *, struct Halo *, int);
    int result = sage_calculate_infall_process(&ctx, &test_halo, 1);
...
int test_physics_calculation_basic(void)
{
    ...
    /* ===== EXECUTE ===== */
    extern int sage_calculate_infall_process(struct ModuleContext *, struct Halo *, int);
    int result = sage_calculate_infall_process(&ctx, &central, 1);
...
int test_satellite_ejected_consolidation(void)
{
    ...
    /* ===== EXECUTE ===== */
    extern int sage_calculate_infall_process(struct ModuleContext *, struct Halo *, int);
    int result = sage_calculate_infall_process(&ctx, halos, 2);
...
int test_multiple_satellites(void)
{
    ...
    /* ===== EXECUTE ===== */
    extern int sage_calculate_infall_process(struct ModuleContext *, struct Halo *, int);
    int result = sage_calculate_infall_process(&ctx, halos, 4);
 ...
 *
 * Executes all sage_calculate_infall comprehensive tests and reports results.
 */
int main(void)
{
    printf("%s", BLUE);
    printf("============================================================\n");
    printf("Test Suite: sage_calculate_infall Module (Comprehensive)\n");
    printf("============================================================\n");
    printf("%s", NC);
