...
int test_event_consumer_beta_process(struct ModuleContext *ctx, struct Halo *halos, int ngal) {
...
int test_event_consumer_beta_cleanup(void) {
  INFO_LOG("test_event_consumer_beta: total events received = %d", events_received);
  events_received = 0;
  return 0;
}
