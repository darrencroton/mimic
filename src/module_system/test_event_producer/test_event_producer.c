...
int test_event_producer_init(void) {
    ...
    INFO_LOG("test_event_producer initialized (emit_count=%d, emit_alt_count=%d)",
             EMIT_COUNT, EMIT_ALT_COUNT);
    return 0;
}

int test_event_producer_process(struct ModuleContext *ctx, struct Halo *halos,
                                int ngal) {
    ...
    for (int i = 0; i < ngal && i < EMIT_COUNT; i++) {
        ...
        int rc = module_emit_event(ctx,
                                   TEST_EVENT_PRODUCER_EVENT_TEST_EVENT,
                                   i, target,
    ...
    for (int i = 0; i < ngal && i < EMIT_ALT_COUNT; i++) {
        ...
        int target = (i + 1 < ngal) ? i + 1 : 0;
        int rc = module_emit_event(ctx,
                                   TEST_EVENT_PRODUCER_EVENT_TEST_EVENT_ALT,
                                   i, target,
                                   (double)total_emitted,
                                   (double)ngal);
        if (rc != 0) {
