...
# Input YAML files
HALO_PROPERTIES_YAML = REPO_ROOT / "metadata" / "halo_properties.yaml"
GALAXY_PROPERTIES_YAML = REPO_ROOT / "metadata" / "galaxy_properties.yaml"
...
def generate_validation_manifest(
    ...
    manifest = {
        ...
        "properties": props,
        "notes": "Auto-generated from metadata/*.yaml. Range is inclusive; sentinels are exempt.",
    }
...
def main():
    ...
    if not HALO_PROPERTIES_YAML.exists():
        ...
        print(
            "Expected location: metadata/halo_properties.yaml",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not GALAXY_PROPERTIES_YAML.exists():
        ...
        print(
            "Expected location: metadata/galaxy_properties.yaml",
            file=sys.stderr,
        )
        sys.exit(1)
