...
def compute_yaml_hash() -> str:
    ...
    for yaml_file in [HALO_PROPERTIES_YAML, MODEL_PROPERTIES_YAML]:
...
def main():
    ...
    if not HALO_PROPERTIES_YAML.exists():
        ...
        print(
            "Expected location: src/core/halo_properties.yaml",
            file=sys.stderr,
        )
        sys.exit(1)

    if not MODEL_PROPERTIES_YAML.exists():
        print(f"ERROR: {MODEL_PROPERTIES_YAML} not found", file=sys.stderr)
        print(
            "Expected location: src/modules/model_properties.yaml",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print(f"  Galaxy properties: {MODEL_PROPERTIES_YAML.relative_to(REPO_ROOT)}")
    print()
    ...
    with open(MODEL_PROPERTIES_YAML) as f:
