...
# validation, spin/Len conventions, fix_upid equivalent (reference semantics, D12)
...
# Phase 4: emit snapshot_NNN.h5 + forests.h5 per docs/dev/HORIZONTAL-HDF5-FORMAT.md
...
<workdir>/
  ...
  hdf5/
    snapshot_NNN.h5        emitted dataset, one file per a_list snapshot (empty
                           snapshots included), per docs/dev/HORIZONTAL-HDF5-FORMAT.md
...
The converter scale-engineering pass (joint review F-13/D4) rebuilt the machinery these notes used to defer to a future production pass, and its acceptance gate ran green on 2026-08-28 — see "The 2026-08-28 acceptance gate" below. Its frozen implementation plan is complete and, like every completed plan here, archived out of `docs/dev/` to `archive/dev-plans/` (gitignored local history); this section therefore names each change by its **landing commit** rather than by a plan slice, so it stays readable without it. The landing commits are `184424df`, `36e17512`, `9ad19662`, `3d52446c`, `c5573d0c`, `ce2a3cf2`, `b3368a8d` and `b2ae9601`, plus `0ab453fe` (the identity-multiplier search, landed early and independently). This section is what each stage does now and what is still sized for the dataset.
...
- **The cross-check is a micro-Uchuu-scale gate, not a production-scale instrument**, and the rehearsal subset is the largest scale it is ever run at: its reference side is a vertical `halos-only` run over the same data, and the ctrees reader preallocates 152,000 B per tree for a whole forest before reading a halo (`src/io/vertical/read_ctrees_ascii.c:647-655`), so the production super-forest's 104,845,278 tree roots alone put ≈15.9 TB of reader preallocation between it and a production reference artifact. No cross-check artifact belongs in a production conversion's storage envelope.
...
**Measured stage-by-stage on micro-Uchuu, 22,580,924 halos** (measured for `b2ae9601`, the commit that introduced the deletion). Peaks are a 1 Hz sampler's maximum over the workdir, so the fast stages carry a few B/halo of run-to-run jitter; the table quotes the maximum observed across all runs.
...
- The converter's own suite passes at that commit: **609 tests, exit 0** (re-measured 2026-09-22; recorded here as 610 in error) (`mimic_venv/bin/python -m unittest discover -s scripts/convert/tests`), up from 327 at the 2026-08-03 re-validation.
...
**Whole forests, always.** `fix_upid` works with per-forest scope, so a
