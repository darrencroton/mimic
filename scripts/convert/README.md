...
**Status:** phases 0–2 implemented (scatter, sort/index — plan Slices 2–4). Fix-ups, links, HDF5 emission, validation, and the micro-Uchuu cross-check land with later slices.
...
**Shin-Uchuu-scale notes (production conversion, out of scope here):** the Phase 0 forest map is currently passed to each pool task by pickling — at the ~5 GB Shin-Uchuu map size that needs a worker initializer with shared or memory-mapped storage; per-chunk per-snapshot boolean scans, whole-file concat reads, and the in-memory sort (~350 B/row peak) are likewise sized for micro-Uchuu, with the chunked external-merge fallback deferred by the plan. Concurrent converter invocations on one workdir are not locked.
