...
UniqueGalaxyID multiplier for the consuming simulation package.
...
def recommended_multiplier(max_rank: int, n_forests_total: int) -> int:
    """Smallest power of ten strictly greater than the measured maximum
    HaloRankInForest, floored at the reader default (TREE_MUL_FAC = 1e9);
    the int64 headroom bound is asserted, never silently degraded."""
    multiplier = DEFAULT_MULTIPLIER
    while multiplier <= max_rank:
        multiplier *= 10
    if multiplier * (n_forests_total + 1) > _INT64_MAX:
        raise ConverterError(
            "no valid identity multiplier: {} x (n_forests_total {} + 1) overflows "
            "int64".format(multiplier, n_forests_total)
        )
    return multiplier
