...
- **Unique galaxy ids** — `make_unique_galaxy_id()` in `src/core/build_model.c` computes `forestnr_global = GlobalForestOffset + unit`, range-checks both components, and encodes through `mimic_encode_unique_galaxy_id()` (`src/include/galaxy_id.h:31`), which is `halonr + TREE_MUL_FAC * (forestnr_global + 1LL)`. **The `+ 1` is load-bearing** — it reserves the first multiplier block, so dropping it shifts every id by a whole block and breaks cross-driver identity. Encode through that helper rather than reimplementing the arithmetic. `PARTITION_PER_FILE` readers get `GlobalForestOffset` from the driver's prefix-sum scan over present files; `PARTITION_ENUMERATED` readers publish chunk offsets. Partition ids and MPI task ranks are not part of the identity.
...
The snapshot **driver** does not exist yet, and that determines exactly how much of the reader runs today. Three levels of checking exist, and only the first is on the run path:

- **Configuration validation — runs on every run.** Two-registry `tree_type` resolution, the reader/order compatibility check, the exact `tree_name` literal, and the identity-multiplier rules, all in `src/core/read_parameter_file.c`.
- **Dataset validation (`open_run`) — implemented and unit-tested, no production caller yet.** `snapshot_reader_open_run()` is called only from the fixture unit tests under `simulations/micro-uchuu-snapshot/_tests/unit/`. Nothing in `src/` calls it.
- **Link-range validation (`load_slab`) — same status**: real, tested, and unreached in production.

The run path is `read_parameter_file()` → `init()` → `run_processing_driver()` (`src/core/main.c:372-373`, `:413`), and `run_processing_driver()` FATALs immediately for a snapshot-ordered configuration at `src/core/tree_driver.c:532`. So a snapshot-ordered run **aborts with the dataset never opened**: a missing or corrupt snapshot file produces exactly the same not-implemented error as a valid one. That is the deliberate Phase 4b/5 split, not an oversight — the Phase 5 driver is `open_run`'s intended run-path caller — but do not describe or rely on a snapshot-ordered run as having proven its input readable.
...
Startup validation then reads the resolved reader's declared `processing_order` and rejects any mismatch with `input.processing_order`, whichever registry answered. Consequently `MimicConfig.reader` is legitimately `NULL` for a snapshot configuration, and every consumer must either guard for that or provably run only after the driver's FATAL. Both kinds exist today. `validate_and_postprocess()` consumes it on the configuration path itself (`src/core/read_parameter_file.c:1401-1412`), *before* the driver, and is correctly guarded — it tests both pointers for `NULL` and then branches on `is_tree_reader`. The **unguarded** consumers are `src/io/tree/interface.c` (`:51`, `:88`, `:118`, `:132`), `src/core/tree_driver.c:488`, and both HDF5 output writers (`src/io/output/master_hdf5.c:46-55`, `:76-80`, `:151-152`; `metadata_hdf5.c:582`), all of which sit after `run_processing_driver()`'s abort and so cannot be reached by a snapshot configuration. So no consumer dereferences a `NULL` reader — but that is because the config-path consumer is guarded, not because nothing runs before the driver. **Do not remove those config-path guards**, and making the output writers reader-kind-neutral is part of the snapshot driver's work.

#### What `open_run` validates

Everything in this subsection describes what `open_run` does *when called* — today that is the fixture unit tests only, per the phase split above. The reader validates structure before reading any data, so a non-conforming file is rejected rather than read into a buffer sized from different assumptions. In order, for every snapshot in the configured snapshot list:
...
Two limits are deliberate. The snapshot reader checks the configured multiplier against the dataset's own bounds at `open_run` (`snapshot_identity_bounds_valid()`: the multiplier must exceed every halo rank, and `multiplier × (n_forests_total + 1)` must fit in `int64_t` — the `+ 1` reserves the encoder's forest offset) — so, per the phase split above, that check runs today only under the reader's unit tests; what a run enforces at startup is that the value is positive. And every helper in `src/include/galaxy_id.h` is still hard-coded to `TREE_MUL_FAC` and takes no configured value, so a **tree-ordered** configuration declaring a non-default multiplier is rejected at startup rather than silently emitting ids computed from the compile-time constant. Replacing the encoder is the snapshot driver's work; until then only snapshot-ordered configurations may set a non-default value.
