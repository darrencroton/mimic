...
**Status**: IMPLEMENTED (2026-06-11). All eight batches of §6 landed as individual commits (`9e56e49` … `1758e61`); see §7 for per-batch verification and deviations. B3 was verified against sage-model and resolved as intentional SAGE parity; M3-5 confirmed as a deliberate subset and annotated. Still open: M1-4 (preamble helper — deliberately deferred), M4-2/M4-3 (test header trim; SHAM unit test — additive follow-up work).
...
**B3 — Satellite stripping metal accounting asymmetry. RESOLVED 2026-06-11: inherited SAGE bug, adopted deliberately for parity.** Verified against `sage-code/sage-model/model_infall.c:106-118` — SAGE's `strip_from_satellite` has line-for-line the same structure. Precise analysis (which narrows the original claim below): with `Z = M/G` the two clamps engage together and the transfer is conservative; metals are destroyed only when the metallicity cap is active (`MetalsHotGas ≥ HotGas`) *and* the gas clamp engages, losing `min(s, M) − G`. Full context, trigger algebra, the correct implementation, and the remediation path are recorded in `dev/issues/sage-satellite-stripping-metal-asymmetry.md`; a parity comment marks the code site. Original finding kept below for the record.
...
**Batch 8 (2026-06-11)** — Simulations polish (M7-1, M7-2): the `tree_loader.py` pair unified to the more complete mini-Millennium version (it contained no simulation-specific text; the example uses the shared CI tree data both packages already rely on); the drifted comment in `test_tree_preservation.py` and the assert message in `test_tree_loading.c` neutralized ("this simulation"), making all three file pairs byte-identical mirrors; both package READMEs now state the mirror-maintenance rule for `halo_properties.yaml`, the `.a_list`, and `_tests/`. Verified: full suite green (unit 32/32, integration, scientific incl. byte-identical baseline), `make check-docs` passes.

**Batch 7 (2026-06-11)** — Plots (M5-1, M5-2): AST-driven unused-import removal across all 31 figure files (script preserved at local `archive/batch7_remove_unused_imports.py`; idempotent re-run confirms zero remaining); sage16 `figures/__init__.py` rewritten in the sham registry's cleaner style (direct returns, no stray string-literal pseudo-docstrings, condensed `check_required_properties` docstring) while keeping all sage16-specific label helpers, registry comments, and entries — 268 → ~230 lines. The 9 shared figure files remain byte-identical between packages (verified by `cmp` loop). Verified: plotting unit tests 13/13, plotting integration suite 6/6, full plot generation 22/22 (sage16) and 11/11 (sham) with zero skipped plots, all figure files re-compiled.

**Batch 6 (2026-06-11)** — Test fixture extraction (M4-1): new `models/sage16/modules/_tests/sage_test_fixtures.h` carries the shared counters, `reset_config()` (overridable via `SAGE_TEST_LOCAL_RESET_CONFIG` for the one file with a custom version), `ensure_modules_registered()`, the `set_test_model_parameters` extern, and `free_test_halo()`; 16 test files now include it. Net −346 lines from test files (−284 including the new header). Deviations from plan: per-module `create_test_halo()` builders all stayed local (signatures/bodies genuinely differ); three files were deliberately left untouched — the merger-ordering-parity test (custom reset_config, near-zero savings) and the satellite-stripping and merger-clock tests (self-contained minimal-mock design that does not include the framework header); `test_unit_metallicity.c` stayed independent of the module-registry-coupled header. Zero `TEST_ASSERT`/`TEST_RUN` lines changed (verified by diff grep). Unit tier 32/32 green. M4-2 (`@author`/`@date` header trim) was deferred to keep this diff purely mechanical.

**Batch 5 (2026-06-11)** — Metadata alignment: dropped the redundant `display_name`/`version` from sham's `module_info.yaml` (M3-3/M6-1; both are optional with auto-generated defaults equal to the explicit values, so the generated registry is unchanged). Verified: `validate-modules` passes for sham, zero-warning sham build, and a full `./mimic models/sham/input/sham_mini-millennium.yaml` run completes. M3-5 (the `first_file: 0 / last_file: 15` annotation in `sage16_millennium.yaml`) remains deferred pending user confirmation of intent.

**Batch 4 (2026-06-11)** — Gas-cycle simplification, implemented as specified: `sage_prepare_infall_budget` and `sage_apply_infall` now use `mimic_find_fof_central_index()` instead of hand-rolled scans (M1-1); the duplicated reionization branches collapsed to one with a Type-agnostic debug line (M1-2); missing-central handling unified to silent return-0 with a one-line comment, matching the merger modules (M1-3); the six `INFO_LOG` cleanup messages downgraded to `VERBOSE_LOG` (M1-9). Verified: unit 32/32, integration, scientific incl. byte-identical baseline; zero-warning sage16 + sham builds.

**Batch 3 (2026-06-11)** — Constants & sentinel protocol, implemented as specified: new `models/sage16/shared/sage_constants.h` names the four cross-file literal families (MergTime sentinel protocol, cold-gas yield threshold, metal-ejection Mvir scale, virial-temperature coefficient) with bit-identical values, adopted in `sage_initialise_merger_clock`, `sage_resolve_mergers_and_disruption`, `sage_apply_metal_enrichment`, `sage_calculate_cooling_budget`, `sage_radio_mode_heating`, and `shared/sage_starburst_physics.h`; `MinNumPartSatHalo` moved to file scope with its parity comment (M1-6); MergTime protocol documented at its property definition and the contradictory HaloBaryonFraction sentinel comment fixed (M3-1/M3-2). Verified: unit 32/32, integration, scientific incl. byte-identical baseline; zero-warning sage16 + sham builds; validate-modules and check-generated clean. Note for later: the `make tests` preamble silences `clean`/`generate-test-registry` output (`Makefile:695-696`), making transient preamble failures undiagnosable — src-side follow-up, out of scope here.
