...
**B3 — Satellite stripping metal accounting asymmetry. RESOLVED 2026-06-11: SAGE parity, not a bug.** Verified against `sage-code/sage-model/model_infall.c:106-118` — SAGE's `strip_from_satellite` has line-for-line the same structure: metals computed from the unclamped gas, gas and metals clamped independently, central credited with clamped-gas × metallicity. The non-conservation when a clamp engages is upstream SAGE behavior, faithfully reproduced; a parity comment now marks the site. Original finding kept below for the record.
