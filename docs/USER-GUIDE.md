...
This guide works through that journey using the shipped **sage16 + mini-Millennium** configuration: the SAGE galaxy formation model (Croton et al. 2006, 2016) run on the mini-Millennium simulation. Everything generalises — other simulations, module pipelines, and property metadata produce different scientific behaviour and output schemas — but the workflow is the same.
...
This reads the mini-Millennium merger trees, evolves galaxies through the full SAGE physics pipeline — reionization, gas infall, cooling, star formation, supernova and AGN feedback, mergers — and writes a catalogue for eight snapshots between z ≈ 8 and z = 0. On a laptop it takes well under a minute. Mimic returns exit code 0 on success; treat any non-zero exit code as a failed run.
...
Mimic ships two model packages, and the same workflow applies to any model package you build yourself:

- **[sage16](../models/sage16/README.md)** — the full SAGE galaxy formation model (Croton et al. 2006, 2016) as ~18 independent modules: reionization, infall, cooling, AGN heating, star formation, supernova feedback, disk instabilities, mergers, and satellite physics. Validated to near-bit-parity against the original SAGE code ([parity report](SAGE16-PARITY-REPORT.md)). This is the package to use for science.
- **[sham](../models/sham/README.md)** — a deliberately minimal one-module package that assigns stellar masses from an analytic stellar-to-halo mass relation. It is a proof of concept for the model-package pattern, not a calibrated science model, but it's the clearest starting point for building your own.

To run a non-default model, build for it and use its run file:
...
Each model package's README documents its scientific scope, pipeline, and parameters. For input simulations, the shipped [mini-Millennium package](../simulations/mini-millennium/README.md) is the working example, and a [full Millennium package](../simulations/millennium/README.md) is provided for users with access to the complete tree data. Adding your own simulation is a developer task — see [Adding a New Simulation](DEVELOPER-GUIDE.md#adding-a-new-simulation).
...
If you use the shipped sage16 model in your research, cite the SAGE papers:
