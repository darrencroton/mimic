...
4. **Snapshot driver + cross-format identity gate** (dual-driver Phase 5) on micro-Uchuu. — **← NEXT.** The reader and the regenerated 50-snapshot dataset are both in place; the reader's dataset validation is implemented and unit-tested but still has no run-path caller, so wiring it in is part of this step rather than something already done. `MIMIC-DUAL-DRIVER-PLAN.md` Phase 5 owns the scope and carries the recorded inputs the reader work surfaced (the `virial.c` tree-index coupling, the driver-neutral output seam, the identity-field access decision, and the `galaxy_id.h` encoder replacement).
