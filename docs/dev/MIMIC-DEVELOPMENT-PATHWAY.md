...
**Date:** 2026-07-02 · **last revised 2026-09-04** — P3b and P4 executed and measured, and P5's re-projection stops the sequence before P6; see `SHIN-UCHUU-CONVERSION-PLAN.md` → "The Production Execution Sequence" for the new callout and its evidence.
...
> ### ► NEXT TASK: the Shin-Uchuu **production run is blocked at P5** — decide how P6 gets more memory or a different memory strategy before it can launch
> **This file is the entry point.** The operational sequence lives in [`SHIN-UCHUU-CONVERSION-PLAN.md`](SHIN-UCHUU-CONVERSION-PLAN.md) → **"The Production Execution Sequence"**: **P1–P3 ran to completion 2026-08-29 to 2026-09-03** (22,503,649,037 halos, 2.0 TB emitted, `unique_galaxy_id_multiplier: 20000000000` set and committed at `5506ef88`). **P3b and P4 ran to completion 2026-09-04** — the package is re-pointed at the production dataset and verified, the z=1/z=2 snapshot pair (52/43) is chosen and committed to all four run files (`06d81228`), and the `Spin` pre-run scan passed cleanly (max |Spin| = 416.69, zero non-finite). **P5 does not clear**: the real largest slab (519,342,987 halos at snapshot 34) re-projects to ≈697 GB unmitigated / ≈639 GB mitigated, both above the ≈589 GB ceiling and the ~540 GB actually available on the Mac Studio. **P6 cannot launch as currently planned** — see that section's new callout for the full arithmetic. `MIMIC-DISTRIBUTED-SNAPSHOT-PLAN.md` was checked and does not help: it targets the cluster case, not a single low-memory workstation, and is unbuilt regardless. Remaining once a memory strategy is chosen: **P6** through **P9** (archive the converted trees — independent of the P6 blocker, can run now), then **F1** (`POST-PHASE-5-JOINT-REVIEW.md` §6 item 10 — stays deferred; likely driver-touching P6 fix work should not disturb the regression net it protects) and **F2** (merge to `main`). That section carries every binding constraint with its code evidence.
>
> The subset rehearsal closed 2026-08-26 (`POST-PHASE-5-JOINT-REVIEW.md` §6 item 6), closing §6 items 2, 3 and 9 from measurement; the converter scale-engineering pass closed 2026-08-28 (§6 item 7), gate green on a fresh micro-Uchuu conversion the rebuilt converter itself produced. **§6 item 10 is the only checklist item still open**, and it is deliberately sequenced last as F1. Work runs on the **Mac Studio** (M3 Ultra, 32 cores, 512 GB); `tooarrana1` (251 GB, 4 cores, no Slurm) holds the source and is a streaming host only. See [Step 1](#step-1-shin-uchuu-in-flight).
>
...
| **1.5** | **Production run + science checks** — `sage16` end to end, then HMF and GSMF at z = 0, 1, 2 | **BLOCKED at P5.** P3b and P4 closed 2026-09-04; P5's re-projection (real largest slab 519,342,987 halos, ≈639–697 GB against ~540 GB available) stops the sequence before P6 — needs a larger host or an unspecified memory strategy, decided by the owner. The goal the whole snapshot pathway exists to reach |
