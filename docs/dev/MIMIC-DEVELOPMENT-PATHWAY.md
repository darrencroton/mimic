...
2. **Run the final v1.0 review and optimisation sweep over the restructured core.** This is the current release blocker: a full code review plus the remaining memory, large-simulation, and HDF5-writer optimisation, benchmark freeze, bug hunt, simplification, documentation cleanup, and lint/format pass. Running it *after* Phases 1–3 is deliberate — the sweep then hardens and optimises the *final* architecture instead of polishing a fused `build_model.c` that the extraction would immediately demolish, and the riskiest extraction (Phase 2) is reviewed under maximum scrutiny while the baseline still guards it.
   ...
   **HDF5-writer optimisation — DONE.** The writer now buffers prepared records in a fixed-size per-snapshot buffer that flushes when full and at end of file (`flush_hdf5_buffers` in `src/io/output/hdf5.c`), replacing the per-tree append pattern. This makes HDF5 output as cheap as binary and keeps memory bounded at large scale. Default output is byte-identical for galaxy data; `FieldMetadata` moved to a single `RunProperties/FieldMetadata` table per file (was duplicated per snapshot, over-chunked), and optional gzip is available at runtime via `--compress` (off by default). The remaining sweep items (memory, large-simulation, bug hunt, etc.) are still open.
