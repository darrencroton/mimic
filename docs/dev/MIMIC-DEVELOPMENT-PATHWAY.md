...
> ### ► NEXT TASK: the Shin-Uchuu **production conversion** — every prerequisite is closed; start at step P1
> **This file is the entry point.** The operational sequence lives in [`SHIN-UCHUU-CONVERSION-PLAN.md`](SHIN-UCHUU-CONVERSION-PLAN.md) → **"The Production Execution Sequence"**: steps **P1** (transfer 11.61 TB) through **P9** (archive the converted trees), then **F1** (`POST-PHASE-5-JOINT-REVIEW.md` §6 item 10) and **F2** (merge to `main`). That section carries every binding constraint with its code evidence, including the one that costs ~24 hours if missed — `scatter --pool-size` defaults to `1` and scatters the whole source serially.
>
> Everything before P1 is done. The subset rehearsal closed 2026-08-26 (`POST-PHASE-5-JOINT-REVIEW.md` §6 item 6), closing §6 items 2, 3 and 9 from measurement; the converter scale-engineering pass closed 2026-08-28 (§6 item 7), gate green on a fresh micro-Uchuu conversion the rebuilt converter itself produced. **§6 item 10 is the only checklist item still open**, and it is deliberately sequenced last as F1. Work runs on the **Mac Studio** (M3 Ultra, 32 cores, 512 GB); `tooarrana1` (251 GB, 4 cores, no Slurm) holds the source and is a streaming host only. See [Step 1](#step-1-shin-uchuu-in-flight).
>
...
Work these in order; each step's output is the next step's input. **1.1, 1.2 and 1.3 are closed. The live work is 1.4 and 1.5**, which are executed as steps P1–P9 of `SHIN-UCHUU-CONVERSION-PLAN.md` → "The Production Execution Sequence".
...
| **1.4** | **Full production conversion** — one-time, 11.61 TB → 70 snapshot files (≈2.31 TB) | **LIVE.** Steps **P1–P3** of the execution sequence. Sets the shin-uchuu identity multiplier from the conversion report's measured counts (2 × 10¹⁰, C15) |
| **1.5** | **Production run + science checks** — `sage16` end to end, then HMF and GSMF at z = 0, 1, 2 | **LIVE.** Steps **P4–P9**. The goal the whole snapshot pathway exists to reach |
...
| [`SHIN-UCHUU-CONVERSION-PLAN.md`](SHIN-UCHUU-CONVERSION-PLAN.md) | **Open — step 1** | Converter built and micro-Uchuu-validated; owns the format, algorithm, measured source data, machine/storage decision and the subset-selection design. The production conversion and the memory-projection fallback trigger remain |
