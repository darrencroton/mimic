...
| `MIMIC-DUAL-DRIVER-PLAN.md` | Partially active | Architecture and phased migration for tree-ordered and snapshot-ordered drivers, plus a physics-only embedded engine | **Phase 1 DONE** (commit `56af880`, byte-identical). **Phase 2 (format-neutral inheritance extraction) is next** and has a detailed implementation-notes block under its phase heading. Phase 3 follows; all three are v1.0 work. Phases 4–7 (snapshot reader and driver): after v1.0 is tagged and its baseline refreshed |
