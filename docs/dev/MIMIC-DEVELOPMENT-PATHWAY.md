...
The direction remains consistent with `docs/VISION.md`: Mimic is a physics-agnostic galaxy evolution framework with runtime-configurable physics modules. The next structural work is to finish modularising the core into a clean physics-execution engine, a shared inheritance service, and a driver-neutral output path. That modularisation is the dual-driver plan's Phases 1–3, but it is **no longer treated as post-v1.0 dual-driver work**: it is core architecture that belongs *in* v1.0 and underpins every candidate forward direction. Two such directions are on the table — a snapshot-ordered second driver (the rest of the dual-driver plan) and the model builder — and they are now largely independent: both build on the same Phase 1–2 engine seam, and nothing in the model builder requires the snapshot driver. The model-builder proposal should still not drive near-term architecture except where it reinforces already-important contracts such as stable module interfaces, generated metadata, deterministic stochastic physics, and strong validation gates.
...
| `MIMIC-DUAL-DRIVER-PLAN.md` | Partially active | Architecture and phased migration for tree-ordered and snapshot-ordered drivers, plus a physics-only embedded engine | **Phases 1 and 2 DONE** (byte-identical; see the per-phase Status blocks — Phase 2 extracted `src/core/inheritance.c`). **Phase 3 (driver-neutral output buffering) is next** and is now narrowly scoped to `marshal_processed_halos()`. All three are v1.0 work. Phases 4–7 (snapshot reader and driver): after v1.0 is tagged and its baseline refreshed |
| `MIMIC-MODEL-BUILDER-PLAN.md` | Aspirational planning brief | Long-term requirements for assisted, gate-driven model construction | Post-v1.0 and after a working science-gate prototype exists; builds on the Phase 1–2 engine seam delivered in v1.0 and does **not** strictly require the snapshot driver |
...
1. **Complete the core modularisation (dual-driver Phases 1–3) as part of v1.0 — not after it.** These phases — separate physics execution from output marshalling (Phase 1, **DONE**, byte-identical), extract the format-neutral inheritance service (Phase 2, **DONE**, byte-identical — now `src/core/inheritance.c`), make output buffering driver-neutral (Phase 3, **next**) — are behaviour-preserving refactors of the existing core. They are not snapshot-driver features; they produce the clean physics-execution seam, inheritance service, and driver-neutral output path that every candidate forward direction needs (both the snapshot driver and the model builder). Land them now, while the existing SAGE physics baseline (`models/sage/modules/_tests/test_scientific_sage_physics_baseline.py`) can prove each one byte-identical to current behaviour. Phase 0 (the driver-dispatch seam) is the one genuinely snapshot-driver-anticipatory step in this group; do it last and trivially, or defer it entirely, and do not let it gate v1.0. See `MIMIC-DUAL-DRIVER-PLAN.md` for phase definitions and gates.
...
7. **Rework the model-builder plan into an implementation plan** only after a science-gate prototype has been grounded in working code. Until then it is a requirements brief, not a mandate. Its hard dependency is the Phase 1–2 engine seam (delivered in v1.0), not the snapshot driver.
