...
2. **Complete `CTREES-UCHUU-VALIDATION.md`.** This is the current release blocker. The repository has the readers, unit/helper coverage, and three micro-Uchuu simulation packages (`micro-uchuu-hdf5`, `micro-uchuu-ascii`, `micro-uchuu-lhalo`) covering all three supported formats. Per-package smoke tests are in place and skip cleanly until data is available. The remaining evidence is an end-to-end run on NT against the real trees, checking dataset packaging, units, topology, MPI partitioning, HDF5 forest distribution, and ASCII/HDF5 parity. Steps: create the `snapshots/` symlinks (see each package's `snapshots.txt`), build `make MODEL=halos-only SIMULATION=<package>`, run, and tick off the checklists in §3, §4c, and §7.
