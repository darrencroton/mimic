...
| `MIMIC-SNAPSHOT-READER-PLAN.md` | **Active — frozen, ready to execute** | The five-slice implementation plan for sequence item 3 (dual-driver Phase 4b): the `SnapshotReader` vtable, its registry, the `snapshot_hdf5` reader, the fixture package, and startup wiring. Derives its scope from `MIMIC-DUAL-DRIVER-PLAN.md` and corrects that plan's Phase 4b/5 boundaries in its Slice 5 | Ready to execute under `project-manager` Mode B; reviewed three times 2026-08-03, both operator preconditions discharged 2026-08-03 |
...
3. **Snapshot reader** (dual-driver Phase 4b) against the micro-Uchuu fixtures. — **← NEXT.** Steps 1–2 are done and the regenerated dataset is in place; the converter's validated micro-Uchuu HDF5 output is the reader-development fixture. Implementation is owned by the frozen [`MIMIC-SNAPSHOT-READER-PLAN.md`](MIMIC-SNAPSHOT-READER-PLAN.md) (five slices, Mode B), which derives its scope from `MIMIC-DUAL-DRIVER-PLAN.md` Phase 4b.
