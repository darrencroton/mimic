...
> ### ► NEXT TASK: the Shin-Uchuu subset conversion and rehearsal — **unblocked 2026-08-25, ready to execute**
> `POST-PHASE-5-JOINT-REVIEW.md` §6 **item 6**. Every runtime prerequisite is closed, the default-pair suite is green, and the operator is now at the conversion machine — the **Mac Studio** (M3 Ultra, 32 cores, 512 GB), which is decided and recorded as the host for everything needing memory or CPU (two small streaming steps run on the data node). The source data is confirmed readable at `/fred/oz214/simulations/uchuu/shinuchuu` on `tooarrana1`, which is a data node (251 GB, 4 cores, no Slurm) and **not** a conversion candidate. The last undesigned piece — how to choose and extract a subset meeting the D9 composition constraint, which four documents required and none specified — was designed on 2026-08-25. **The operational sequence is `HANDOFF.md` at the repository root**, which is self-contained through to the production conversion. Like every handoff here it is **gitignored by design** — it lives only on the conversion machine, because that is where the work happens; the design it executes is committed here. See [Step 1](#step-1-shin-uchuu-in-flight).
>
...
|---|---|---|---|
| **1** | **Shin-Uchuu** — production conversion and `sage16` end to end | root `HANDOFF.md` (gitignored, conversion machine) for execution; [`SHIN-UCHUU-CONVERSION-PLAN.md`](SHIN-UCHUU-CONVERSION-PLAN.md) for design; checklist at [`POST-PHASE-5-JOINT-REVIEW.md`](POST-PHASE-5-JOINT-REVIEW.md) §6 | See [Step 1](#step-1-shin-uchuu-in-flight). **Unblocked 2026-08-25** — execute the handoff |
...
| **1.4** | **Full production conversion** — one-time, 11.61 TB → 70 snapshot files (≈2.29 TB) | Sets the shin-uchuu identity multiplier from the conversion report's measured counts |
...
**Subset selection was designed 2026-08-25 and is the rehearsal's first task.** The composition constraint below had been stated in four places and operationalised in none; `SHIN-UCHUU-CONVERSION-PLAN.md` → "Subset Selection and Extraction" now specifies the whole route — rank forests by size from `forests.list` + `locations.dat` alone (21.3 GB), sample one root row per candidate tree remotely to get exact masses (~1 GB of scattered reads), select a random stratum plus a high-mass supplement, extract whole forests by streaming byte ranges on the data node, and verify before transferring. The bulk 11.61 TB is never read for selection. Build it as `scripts/convert/subset.py`.
...
**Re-measured 2026-08-25, and three figures moved.** `mergertrees/` is **11.61 TB apparent** across 2,744 `tree_*.dat`, not 5.6 TB — `du` under-reports `st_blocks` on that Lustre filesystem, and the data was NUL-probed to confirm it is dense rather than sparse. `forests.list` is **7.56 GB**, not 17 GB (its 24.0 B/line width corroborates the 315,004,242 tree count exactly). Total halos are **≈22.9 billion** at a measured 506.3 B/row, not 15–18 billion at an assumed 350. Consequences are recorded in the conversion plan's Feasibility section: output HDF5 ≈2.29 TB, the shipped in-memory rank pass ≈1.10 TB (2.1× installed RAM, so step 1.3's external-merge sort is mandatory by a wider margin), and transfer ≈29 h at a **measured** 110 MB/s. **None of it touches the rehearsal**, whose rank pass needs ~17.5 GB.
...
|---|---|---|
| root `HANDOFF.md` | **Open — step 1, executable. Gitignored by design** | Self-contained operational sequence from a cold start through the subset rehearsal to the production conversion's start line. Written 2026-08-25. It lives only on the conversion machine; everything it depends on is committed. The superseded one is at `archive/handoffs/` |
