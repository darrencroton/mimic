...
- **The baseline mechanism already exists.** `tests/data/output/baseline/` (binary + HDF5) is captured Mimic output used as a byte-regression reference by `tests/integration/test_output_formats.py`. This is a *Mimic-vs-Mimic* check — exactly the right tool for the behaviour-preserving gates here. Extend/refresh it against the tagged v1 and diff every phase against it. This same baseline has additionally been verified **byte-identical to sage-model**, so it doubles as a trusted external reference (a fact `docs/galaxy-model-builder-design.md` relies on for its reference-parity gate).
...
- The generated output **schema**, provenance, and binary/HDF5 writers (`src/io/output/*`, `main.c:189` schema writer, HDF5 attrs `:476–489`) are unchanged — only the producer of the records becomes pluggable.
...
**Goal:** read the snapshot-ordered file format into an in-memory snapshot-grouped model with descendant/progenitor links.
...
- No driver yet — unit-test the reader in isolation (counts, link integrity, round-trip against a converter-produced fixture).
...
- **Global-operation hook (the payoff):** because a snapshot's population is co-resident, a per-snapshot collective step is now expressible (global ranking for SHAM, occupation statistics, a shared radiation field, environment fields). Introduce it as an **opt-in per-snapshot collective hook** that modules declare they need; models that do not use it pay nothing.
