...
- **Not open, but must be folded in when this work touches `scatter.py`:** `scatter.py:772` opens one write handle per snapshot with Python's default buffering, which some filesystems (Lustre stripes, some APFS temp volumes) report a large `st_blksize` for — inflating each handle's buffer well beyond what "bounded by snapshot count" implies, up to `n_snapshots` concurrent handles. `crosscheck.py`'s `TopologyDumpPartition._handle()` had the identical defect (found and fixed 2026-09-17 chasing a converter-test memory regression) with an explicit `buffering=` size; `scatter.py`'s worker scratch writer never got the same fix since it was out of scope that day. Apply the same fix here — it is real production exposure at Shin-Uchuu's own scale (up to 70 concurrent handles), not merely test-tuning noise.
