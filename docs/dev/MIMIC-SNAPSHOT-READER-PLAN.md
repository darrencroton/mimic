...
**Status: EXECUTED AND COMPLETE — all five slices implemented and accepted, 2026-08-03 to 2026-08-04.** Retained rather than archived because its [Deferred and Out of Scope](#deferred-and-out-of-scope-recorded-so-nothing-is-silently-dropped) section is the input list for Phase 5; archive it once Phase 5 closes. Executed under `project-manager` Mode B on branch `feature/ctrees-snapshot-reader`, Developer `claude-opus-5`, with an independent drift audit and two independent code reviews (`codex gpt-5.6-sol`, `opencode deepseek-v4-flash`) commissioned per slice. Outcome: Slices 1, 3 and 4 accepted with no correction; Slice 2 took one correction round (an HDF5 attribute byte-order defect, an `hsize_t`→`int` narrowing, and an unguarded name in diagnostics); Slice 5 took two (documentation of record claimed a snapshot-ordered run validates its input dataset when the reader has no run-path caller, and stated the `UniqueGalaxyID` formula without its `+ 1` forest offset). Every gate in the Definition of Done was met, including the byte-identical tree-ordered comparison and the 50-snapshot real-data open over 22,580,924 halos. The plan itself needed one operator-authorized amendment mid-run (Revision 5 below) after two acceptance criteria proved unsatisfiable by any in-contract implementation.

Original planning status, retained for provenance: frozen implementation plan for pathway item 3 (snapshot reader). Planned 2026-08-03 against `feature/ctrees-snapshot-converter` at `1c91f2b1`. Revision 5. Revision 3 followed three independent read-only review rounds by Codex `gpt-5.6-sol` (xhigh effort), each finding independently verified against the repository before adoption. Revision 4 (2026-08-03) records that both operator preconditions are discharged (see [Preconditions](#preconditions)); the only change to frozen slice content is that Slice 4's real-data test becomes a required check rather than an accepted SKIP, which strengthens a gate. No slice scope, authorized surface, or non-goal changed.
...
**Recorded but deliberately NOT amended.** Two defects were found and left in place rather than amended, both carried as follow-ups in [Deferred and Out of Scope](#deferred-and-out-of-scope-recorded-so-nothing-is-silently-dropped):

1. The Invariant 5 enumeration in Slice 2 names only "sentinel carried with halos present aborts" and "zero forest count with a non-empty dataset aborts", and never the third combination — a wholly empty dataset carrying non-sentinel metadata — which the shipped reader therefore accepts. Specifying it would add validation to an already-accepted slice.
2. Slice 4's "Functions/classes/components allowed to change" list omits `read_parameter_file()`'s own body, while its Intended Change *requires* seeding the identity multiplier "exactly once, before either parse" — which can only happen inside that body. A literal reading of the components list therefore makes a mandated edit impossible. The implementation was accepted on the only implementable reading (the seeding block is authorized), after verifying the change is provably neutral for production: `INPUT_PROCESSING_ORDER_TREE == 0`, `MimicConfig` is a zero-initialised file-scope global (`src/core/allvars.c`), and `read_parameter_file()` is called exactly once (`src/core/main.c:372`), so the seed writes values already present — confirmed empirically by the byte-identical binary output. **If this plan is ever reused as a template, add that seeding block to the components list.**
...
No payload populator of any kind is in this plan. At the time this was written the deliverable was named `populate_halo_payload_from_snapshot.inc` and assigned to **Phase 5**, resolving a contradiction in `docs/dev/MIMIC-DUAL-DRIVER-PLAN.md` (`:135` assigned it to the snapshot driver while `:147` listed it under Phase 4b) in favour of `:135`. **Superseded 2026-08-04:** Phase 5's explicit-view decision removed the tree coupling that justified a second populator, so **no `_from_snapshot` sibling will ever exist** — there is one shared view-based generated populator. See `MIMIC-DUAL-DRIVER-PLAN.md`'s Phase 4b correction note and recorded input 1. What remains true is the original point: Phase 4b produced no populator of any kind.
...
- **Payload populator and snapshot accessor family** — Phase 5. **Superseded 2026-08-04:** this entry originally named a `populate_halo_payload_from_snapshot.inc` sibling; Phase 5's explicit-view decision means there is **one shared view-based populator** and no `_from_snapshot` file (`MIMIC-DUAL-DRIVER-PLAN.md`, recorded input 1). Whatever generated file results still requires an entry in `scripts/check_generated.py`'s hand-maintained list, or it will silently drift.
...
- **Converter test-tier integration** — deferred by the converter plan until the snapshot reader exists; now unblocked, but out of this plan's scope. **Done 2026-08-10:** `make tests-converter` runs the stdlib-unittest suite and `make tests` includes it.
- **Package-dependent assertions in core unit tests (Revision 5, structural — the most important item here).** Core tests under `tests/unit/` are driven by the generated `build/generated/test_inputs/<model>/<sim>/core/test_binary.yaml`, whose simulation configuration is the **selected** package's `simulation_info.yaml`. Any core test asserting a "default" is therefore asserting the selected package's value. Revision 5 fixes the two instances this plan tripped over in `test_parameter_parsing.c`; it does **not** sweep the rest. Phase 5 adds further non-tree-ordered packages and will keep tripping this, so Phase 5 should begin with a sweep of `tests/unit/` for assertions that look package-independent but are not. **Done 2026-08-10 (pre-Phase-5):** the sweep ran and found exactly two further unsafe instances, both in `tests/unit/test_parameter_parsing.c`, both fixed; see `docs/dev/PHASE-4B-REVIEW-AND-PRE-PHASE-5-WORK.md` W3.
...
- **Identity-bounds wording in the Developer Guide.** `docs/DEVELOPER-GUIDE.md` (the `simulation.unique_galaxy_id_multiplier` paragraph) paraphrases `snapshot_identity_bounds_valid()`'s overflow half as "`forest_index × multiplier` must stay inside `int64_t`". The implemented bound (`src/io/snapshot/interface.c`) is `n_forests_total > INT64_MAX / multiplier - 1`, i.e. **`multiplier × (n_forests_total + 1)` must fit in `int64_t`** — the `+ 1` reserve is dropped in the paraphrase. The rank half of the sentence is correct, and the exact bound is stated correctly in both `SNAPSHOT-HDF5-FORMAT.md` and this plan's correction to `MIMIC-DUAL-DRIVER-PLAN.md`, so nothing is materially wrong. **Done 2026-08-10:** the guide and the same loose paraphrase in `src/io/snapshot/reader.h` both state the exact bound now.
...
- **Fixture-tooling and test-harness hardening.** `simulations/micro-uchuu-snapshot/_tests/input/create_snapshot_fixture.py`'s `regenerate()` is not transactional — it unlinks the committed `.h5` set before rewriting, so a mid-run failure leaves a partial set beside a stale manifest, contradicting `FixtureError`'s own docstring; `check_fixture_conformance.py`'s `check_filters()` never asserts chunkedness while its docstring implies the chunked contract is enforced; neither the conformance checker nor the generator is wired into any test target, so the committed fixture is protected only by manual re-runs; `expect_fatal()` in the fixture unit test drains at most 16383 bytes of child stderr and then closes the pipe's read end before `waitpid`, so a future child emitting ≥16 KB dies of SIGPIPE and fails the case loudly with truncated capture — a spurious needle-miss failure, not a deadlock (an earlier revision of this entry misdescribed it as one); the comparison buffers there are hard-sized from `FIXTURE_MAX_HALOS`, with no assertion tying them to the loaded slab; and the link-range validator reads members via `*(const int *)` with `offsetof` and no `_Static_assert` tying those members to the generated width, so a future package typing a link field wider would silently validate a truncated value. All are latent or unreachable under the frozen contract and none is a Phase 4b defect. **Done 2026-08-10 (pre-Phase-5):** all of the above hardened — transactional regeneration, a chunkedness assertion, `make check-snapshot-fixture` wired into `make tests`, truncation-aware `expect_fatal`, extent guards on the fixed buffers, and `_Static_assert`s on the five link fields.
