...
**Status:** Frozen implementation plan for pathway item 3 (snapshot reader). Planned 2026-08-03 against `feature/ctrees-snapshot-converter` at `1c91f2b1`. Revision 3, after three independent read-only review rounds by Codex `gpt-5.6-sol` (xhigh effort), each finding independently verified against the repository before adoption.
...
2. **Restore the converter's Python dependency — REQUIRED BEFORE THE RUN STARTS.** `pandas>=2.0.0` is declared at `requirements.txt:6` but is *not* installed in this machine's `mimic_venv` (verified 2026-08-03: `ModuleNotFoundError`). Slice 1's fixture generator cannot run without it:
   `mimic_venv/bin/pip install -r requirements.txt`
3. **Full micro-Uchuu dataset regeneration — OPTIONAL FOR THIS RUN, out of plan.** The converter's `output/convert/micro-uchuu/` workdir no longer exists on this disk; the 12 GB ASCII source at `/Volumes/Internal/data/uchuu/micro-uchuu/micro-uchuu-ascii/` is intact. Re-run the converter using the commands in `scripts/convert/README.md`, place the 50 `snapshot_NNN.h5` files plus `forests.h5` at `/Volumes/Internal/data/uchuu/micro-uchuu/micro-uchuu-snapshot/`, then create the machine-local symlink:
   `ln -s /Volumes/Internal/data/uchuu/micro-uchuu/micro-uchuu-snapshot simulations/micro-uchuu-snapshot/snapshots`
   (`simulations/*/snapshots` is covered by `.gitignore:7`.) This is required before Phase 5. Slice 4's opt-in real-data test uses it when present and **SKIPs with a reason when it is absent — that SKIP is an accepted Slice 4 outcome for this run.** Every other gate in this plan passes without it.
...
- `tests/unit/run_tests.sh`: add `src/io/snapshot/interface.c` and `src/io/snapshot/registry.c` to the **unconditional** `IO_SRCS` list at `:154`; add `src/io/snapshot/read_snapshot_hdf5.c` to the HDF5-only group at `:156`; add `snapshot/registry.c` and `snapshot/read_snapshot_hdf5.c` to the per-source `-DHDF5` `case` list at `:191-198`; add the new test name to the HDF5-availability skip check at `:292-293`.
...
### Acceptance Criteria
- Inputs: run YAMLs and simulation packages; the fixture dataset; optionally the full converted dataset (precondition 3).
...
- [ ] The opt-in real-data test opens all 50 snapshots and reports their halo counts when the dataset is present, and emits exactly one `MIMIC_RESULT: SKIP` naming the missing path when it is not. Either outcome is acceptable for this run.
...
- Files allowed to change:
  ...
  - `tests/unit/run_tests.sh`
- Functions/classes/components allowed to change: `parse_input_section`'s reader resolution, `parse_simulation_section`'s key list and parsing, the config validation block at `read_parameter_file.c:1354-1368`, `MimicConfig`, and new snapshot-interface helpers. In `tests/unit/run_tests.sh`, **only** the HDF5-availability skip-name check at `:292-303`, to register the new HDF5-dependent real-data test; no other line of that script may change in this slice.
- Tests allowed or expected to change: `tests/integration/test_processing_order.py` and the fixture package's C unit tests.
...
- [ ] Behaviour that must not change: no code, YAML, test, or fixture change of any kind; the full default-pair suite is green and unchanged.
...
### Explicit Non-Goals
- No code, YAML, Makefile, or test change.
...
- **Full micro-Uchuu dataset regeneration** — operator precondition 3; required before Phase 5, and turns Slice 4's opt-in test from a SKIP into a real check.
...
Confirm precondition 2 is satisfied before starting. Record whether precondition 3 is present; its absence is acceptable and requires the specified SKIP in Slice 4.
