...
- **Identity-bounds wording in the Developer Guide.** `docs/DEVELOPER-GUIDE.md` (the `simulation.unique_galaxy_id_multiplier` paragraph) paraphrases `snapshot_identity_bounds_valid()`'s overflow half as "`forest_index × multiplier` must stay inside `int64_t`". The implemented bound (`src/io/snapshot/interface.c`) is `n_forests_total > INT64_MAX / multiplier - 1`, i.e. **`multiplier × (n_forests_total + 1)` must fit in `int64_t`** — the `+ 1` reserve is dropped in the paraphrase. The rank half of the sentence is correct, and the exact bound is stated correctly in both `SNAPSHOT-HDF5-FORMAT.md` and this plan's correction to `MIMIC-DUAL-DRIVER-PLAN.md`, so nothing is materially wrong. Tighten the guide's wording to the exact bound whenever that section is next edited.
