...
**Status:** CLOSED — R1–R15 all complete, Definition of Done 0–11 satisfied. This is the remediation plan for a scientific-correctness defect found in the Shin-Uchuu production dataset during P8's science-validation pass.
**Date:** 2026-09-09, last revised 2026-09-17 (closeout).
**R8–R13 done 2026-09-16; R14–R15 done 2026-09-17.** The v2 production re-conversion ran to completion (5d18h35m wall clock), `report` passed (`validation_passed: true`, `flyby_demotions: 0` everywhere), and post-conversion checks confirmed both new acceptance criteria on direct measurement of the emitted dataset: **z=0 FoF-central count 244,953,607 (exact match)** and **zero negated `MostBoundID`**. `simulations/shin-uchuu/snapshots` now points at `production-snapshot-v2`, verified against the report for all 70 snapshots with zero mismatches. The `sage16` production run on the corrected dataset then passed all four of R13's numerical acceptance criteria exactly. **R14 (retire v1 artefacts) and R15 (close out) are now also done** — see their table rows below for the verified destinations and byte counts. This closes the remediation; see [Definition Of Done](#definition-of-done) for the final confirmation of all 12 items.
...
**Entry point.** The live root `HANDOFF.md` tracked this work through R14/R15; now that the remediation is closed, it has been archived to `archive/handoffs/HANDOFF-2026-09-17-flyby-defect-remediation-complete.md` per `MIMIC-DEVELOPMENT-PATHWAY.md`'s standing rule (a handoff is gitignored and never citable as the source of a technical fact — every fact it held either lives in this document already or has been folded in below). This document, indexed from `MIMIC-DEVELOPMENT-PATHWAY.md` → Step 1, is now the sole entry point for this work's history. For what comes next, see that pathway document's "CURRENT FOCUS" banner.
...
`fix_flybys()` — `src/io/vertical/ctrees/ctrees_utils.c:318-412`, called from `src/io/vertical/read_ctrees_ascii.c:693`:
...
- **The vertical driver** visits every unprocessed halo in a forest (`src/core/vertical_driver.c:228-250`), and `build_halo_tree()` processes each halo's actual FoF chain rather than treating the forest as one group (`src/core/build_model.c:109-129`, `:137-179`).
- **The horizontal driver** enumerates every self-central in the slab and processes each FoF chain separately (`src/core/horizontal_driver.c:912-929`).
...
- `src/io/vertical/ctrees/ctrees_utils.c`, `ctrees_utils.h` — `fix_flybys()` itself and its declaration/provenance comment.
- `src/io/vertical/read_ctrees_ascii.c` — the call site (`:692-695`) and the pipeline comment (`:24-25`).
...
- `docs/dev/HORIZONTAL-HDF5-FORMAT.md` — Ordering Contracts item 5, Format Invariant 3's sign clause, the "reference semantics" definition (`:124`), the `MostBoundID` dataset description.
- `src/io/horizontal/read_horizontal_hdf5.c` (`HORIZONTAL_HDF5_FORMAT_VERSION` at `:62` and its four error-message uses), `src/io/horizontal/reader.h`, `simulations/micro-uchuu-horizontal/halo_properties.yaml`.
...
- `simulations/micro-uchuu-horizontal/_tests/input/create_snapshot_fixture.py:583-615` (requires a negative id), `check_fixture_conformance.py`, `_tests/unit/test_unit_horizontal_reader_open.c`, `test_unit_horizontal_reader_realdata.c`, `_tests/data/fixture_manifest.json`, and the committed `_tests/data/snapshot_00{0..5}.h5`.
...
- `simulations/{micro-uchuu,micro-uchuu-ascii,micro-uchuu-hdf5,micro-uchuu-horizontal,shin-uchuu,shin-uchuu-ascii}/README.md`. (`shin-uchuu-ascii`'s carries no flyby claim today — include it for completeness, since it is one of the four exposed packages.)
...
- The **cross-format identity gate** (`docs/DEVELOPER-GUIDE.md:1072-1078`) compares ASCII against horizontal-HDF5 *converted from that same ASCII reference*, and the converter's topology cross-check includes the signed `MostBoundID` and the five rewritten link fields (`scripts/convert/README.md:516-522`). **Both prove parity with the reference, not scientific correctness** — that is precisely why neither caught this. Removing `fix_flybys` from both sides keeps them meaningful, but the `micro-uchuu-horizontal` fixture dataset must be regenerated first or the gate compares v1 data against a v2 reader.
- The producer battery checks index ranges and structural chain consistency, never whether the FoF grouping matches the source catalogue (`HORIZONTAL-HDF5-FORMAT.md:103-112`, `:132-136`). §10 item 8 adds the check that would have caught this.
...
2. **The scale dependence was not anticipated.** Nothing in the record connects "forests percolate harder at finer particle mass" to "the flyby fix's blast radius grows with resolution". The percolation super-forest was known — `HORIZONTAL-HDF5-FORMAT.md` even sizes `HaloRankInForest` as int64 because of it — but it was treated purely as a *memory and indexing* problem, never as a *topology semantics* problem.
...
### D3 — Bump the horizontal format to `format_version = 2`

Removing the flyby demotion changes which files on disk conform. By `HORIZONTAL-HDF5-FORMAT.md`'s own rule (line 168) that is not an erratum, it is a version bump. Concretely:
...
- `src/io/horizontal/read_horizontal_hdf5.c` must accept 2 and reject 1, so the **existing 2.25 TB dataset fails fast** rather than being silently re-used. This is a feature: it makes the bad dataset unusable by construction.
- The `micro-uchuu-horizontal` fixture dataset and its committed test fixtures must be regenerated at version 2 before the identity gate is meaningful.
...
The in-place repair is *technically* narrower than it first appears, and the reason is worth recording. The emitted slab is ordered by ascending `|MostBoundID|` (Format Invariant 3, `HORIZONTAL-HDF5-FORMAT.md:103-110`; `scripts/convert/links.py:37-42`), verified on all five sampled snapshot files (§3.3) — so clearing the sign does not move a row. And `HaloRankInForest` is **identity metadata, not a slab index**: `src/core/horizontal_driver.c:363-385` states that explicitly, and `Descendant`/`FirstProgenitor`/`NextProgenitor` are the slab indices (`HORIZONTAL-HDF5-FORMAT.md:70-101`). So repairing `snapshot_069.h5` would preserve row order and leave every descendant and progenitor index untouched. Only four datasets in one file would change: `MostBoundID` (un-negate), `FirstHaloInFOFgroup`, `NextHaloInFOFgroup`, and `HaloRankInForest` (a permutation *within* each forest's z=0 block, so `max_halo_rank_in_forest` and `ForestIndex` are unchanged). `UniqueGalaxyID` is a **runtime** value the reader composes from `ForestIndex` and the rank (`HORIZONTAL-HDF5-FORMAT.md:126-130`) — it is not a field to patch, and it changes for galaxies created at snapshot 69 as a consequence, not as an extra task.
...
**Sequencing rule.** The column decision touches the same files as the `fix_flybys` removal and the same format bump (`ctrees_parser.py`, `fixups.py`, `hdf5_writer.py`, `read_ctrees_ascii.c`, `HORIZONTAL-HDF5-FORMAT.md`, the two shin-uchuu `halo_properties.yaml`, the micro-uchuu-horizontal fixtures, the producer battery and the cross-check). Taking it **first** means one code change, one acceptance gate, one commit. Taking it later means doing all of that twice. Hence R2 before R3.
...
**The D6 coupling — real, but smaller and more conditional than first stated.** An additional `float32` source column widens `struct RawHalo` by 4 B/halo. The horizontal driver holds **two live slab generations**, so on the largest slab (519,342,987 halos) the raw-slab cost is
...
- **C reader**: `verify_fof_centrals_present()` in `src/io/vertical/ctrees/ctrees_utils.{c,h}`, called from `src/io/vertical/read_ctrees_ascii.c` at the point `fix_flybys` used to occupy, before `fix_upid`. It scans the forest's `additional_info` for the maximum `scale`, counts `pid == -1` records at that scale, and `FATAL_ERROR`s (via the caller) naming the forest/unit id and the offending scale if the count is zero.
...
| **R8** | **DONE 2026-09-16.** All 4 batches (685/686/687/686 files) transferred from NT and verified byte-exact against the recorded manifest sizes, reusing the frozen batch plan and `forests.list` unchanged from v1 per D6/R2. One transient failure: batch 1's transfer dropped after 4h56m (`Connection to nt.swin.edu.au closed by remote host`, rsync exit 255, the documented NT-flakiness risk) with 481/685 files already landed; the driver's built-in retry (5 attempts, exponential backoff) resumed and completed the remainder in 2h38m — rsync's own size/mtime check skipped the already-complete files, no data was re-transferred. Batches 2–4 transferred cleanly on the first attempt. Total P1 wall clock across all 4 batches: ≈29h51m (transfer only, interleaved with R9's scatter/release). The original charge follows as the record. **P1 — transfer the 11.61 TB source from NT, batched.** `rsync -aL --files-from=<batch list>`, **no `--checksum`**, 5 attempts with backoff; confirm the 7.56 GB production `forests.list` is still at `/Volumes/LaCie/staging/shin-uchuu/forests.list` — §6 records it as retained and R1 does not sweep it — and re-fetch only if it is gone (it is a hard prerequisite of the first `scatter`). Re-cut the batch plan **only if** R2's re-derived envelope stopped clearing the 7.0 TB ceiling. Interleaved with R9 batch by batch, exactly as before: transfer → scatter → release → delete → next. | YES |
| **R9** | **DONE 2026-09-16.** Full pipeline ran to completion on `WORKDIR=/Volumes/LaCie/convert/shin-uchuu-v2`, emitting to `/Volumes/LaCie/data/shin-uchuu/production-snapshot-v2` (2.0 TB, 71 files). Stage timings (`/usr/bin/time` was not wrapped per-invocation — the driver's own wall-clock elapsed is recorded instead, which is equivalent evidence): `finalize` 12h23m, `sort` 8h19m, `fixups` 9h41m (slightly faster than v1's 9h51m, consistent with the deleted `fix_flybys` demotion loop), `links` 28h09m (essentially matching v1's 28h29m — **the ≈235 GB memory-peak stage completed with no crash**), `write` 7h11m, `report` 5h08m. **Total P1+P2 wall clock: 5d18h35m** (2026-09-10 08:29 UTC → 2026-09-16 03:04 UTC), against the ≈5.76-day (138.3h) projection — actual 138.6h. `conversion_report.json`: `validation_passed: true`, `flyby_demotions: 0` in every one of the 70 per-snapshot entries and in the run totals, `max_halo_rank_in_forest = 12,646,607,900` (within the `2×10¹⁰` multiplier bound), `total_halos = 22,503,649,037` — identical to v1, as expected since the fix relabels FoF-central status without creating or destroying halos. The snapshot-span gate passed: all 70 snapshots non-empty. Full log record: the archived root handoff's execution log (`archive/handoffs/HANDOFF-2026-09-17-flyby-defect-remediation-complete.md`, gitignored, not citable, but a faithful blow-by-blow) and, since R15 archived the staging directory whole, `archive/shin-uchuu-run/staging/{state.json,logs/,conversion_report.{json,txt}}` (was `/Volumes/LaCie/staging/shin-uchuu/` at the time this stage ran). The original charge follows as the record. **P2 — production conversion on `/Volumes/LaCie`.** `--workdir /Volumes/LaCie/convert/shin-uchuu-v2`, **a fresh workdir** (the manifest binds to its inputs and refuses to resume across a dtype change — and R2 may have changed the dtype). `scatter --batch --pool-size 8` per batch then `release`, then `finalize` → `sort` → `fixups` → `links` → `write --output-dir /Volumes/LaCie/data/shin-uchuu/production-snapshot-v2` → `report --multiplier 20000000000`. **`--consume-intermediates` on all three of `fixups`, `links` and `write`** — mandatory here, see D5. **Wrap every stage in `/usr/bin/time -l`** and record peak RSS; the original run measured none of them. | YES |
| **R10** | **DONE 2026-09-16.** Multiplier re-confirmed unchanged (`2×10¹⁰`, already committed; `max_halo_rank_in_forest` 12,646,607,900 sits well inside it). `simulations/shin-uchuu/snapshots` re-pointed at `production-snapshot-v2` (`readlink` confirmed) and all 70 snapshot file headers (`n_halos`, `n_forests_total`, `max_halo_rank_in_forest`) verified against `conversion_report.json` — zero missing, zero mismatched, per-snapshot row sum exactly matches the report total. **Full `Spin` scan over all 22,503,649,037 halos**: `max \|Spin\| = 416.69`, **zero non-finite** — a clean pass, comfortably inside `[-1000, 1000]` and even better than the ~680 mass-matched-anchor prediction (723.7 s wall clock). **New checks, independently measured by direct scan of `snapshot_069.h5` rather than by trusting the report's own counters**: z=0 FoF-central count (`FirstHaloInFOFgroup[i] == i`) is **244,953,607 — exact match to target**; negative-`MostBoundID` count is **0**; zero `MostBoundID == 0` too; minimum value 26,551,468,179. P5's memory re-projection was deferred to R11, which subsumes it (the owner directed a local run on the Mac Studio; R11 verifies live memory headroom immediately before launch rather than projecting it here, since the two are separated by however long R11 takes to start). The original charge follows as the record. **Post-conversion checks — P3, P3b, P4, P5.** Confirm `n_forests_total`/`max_halo_rank_in_forest` against the new report and re-confirm the 2 × 10¹⁰ identity multiplier (it should be unchanged, but confirm rather than assume); re-point `simulations/shin-uchuu/snapshots` at the v2 dataset and verify per-snapshot counts against the report; full `Spin` scan; re-project P6's memory peak from the new per-snapshot slab counts, **including any widening R2 introduced**. **New checks this round: assert zero negated `MostBoundID` in every snapshot file, and that the z=0 FoF-central count is 244,953,607 rather than 166,547,771.** | YES |
...
| **R12** | **DONE 2026-09-16, local branch.** R11 chose the Mac Studio (physical memory confirmed 549.76 GB). `output_directory` repointed to a fresh `output/sage16-shin-uchuu-v2` via a copy of the shipped run file (`sage16_shin-uchuu-v2.yaml`), diffed against the original to confirm the output path was the only functional change; `simulations/shin-uchuu/snapshots` and `format_version == 2` both asserted before launch. Ran detached under a wrapper with the D7 memory watchdog: one false-start (an over-sensitive "pageouts rising" rule fired on ~1 MB/60s of ordinary background paging with swap flat; fixed with a magnitude floor and a baseline-relative swap threshold, restarted within ~12 minutes of real time lost), then a clean run — **exit 0, 9h35m04s, peak RSS 513.033 GB** against 549.76 GB physical (36.7 GB headroom), swap increase over baseline only 51 MB across the whole run. **The NT branch was never taken, so its named fix — `ops/nt/sage16_shin-uchuu.sbatch:78`'s symlink target — was never updated.** That script is now doubly stale: it still points at `snapshot-trees`, which R14 renamed to `snapshot-trees-v1` on NT, so an unmodified `sbatch` run today would fail outright on the missing symlink target (a safe failure, not a silent wrong-version selection, but not a working script either). Flagged here rather than fixed, since fixing it wasn't exercised or validated this round — whoever next needs the NT branch (fallback for a future run, or larger-than-local-memory work) should apply this row's original fix before submitting. The original charge follows as the record. **P6 — the `sage16` production run, on the host R11 chose.** Whichever branch, **a fresh output directory is mandatory**: `models/sage16/input/sage16_shin-uchuu.yaml:11` writes to `output/sage16-shin-uchuu`, HDF5 output truncates on create (`src/io/output/hdf5.c:88-91`), and that path resolves to the **retained v1 output**. Set `output_directory: output/sage16-shin-uchuu-v2` and assert the directory is empty before launch.<br><br>**Local branch:** create `output/sage16-shin-uchuu-v2` (on LaCie, symlinked like its v1 sibling) and confirm ≥ 600 GB free there. Assert `simulations/shin-uchuu/snapshots` resolves to the v2 dataset and that a snapshot file reports `format_version == 2`. Launch detached through a wrapper that records the true exit code. **Start the D7 monitor before the run and keep it running to completion** — 60 s samples of `vm.swapusage` and `vm_stat` to a log; abort on swap > 2 GB or `Pageouts` rising across three consecutive samples; the projected peak is at snapshot 34, mid-run, so a quiet first hour proves nothing. On abort, fall back to the NT branch.<br><br>**NT branch:** pull the checkout to the R7 commit. `ops/nt/sage16_shin-uchuu.sbatch:78` unconditionally does `ln -sfn …/snapshot-trees simulations/shin-uchuu/snapshots` — **it would silently select v1**, which the v2 reader then rejects (`read_horizontal_hdf5.c:1056-1059`). Change it to `snapshot-trees-v2`, and add an assertion on the resolved target plus a `format_version == 2` check alongside the existing 71-file count check. Archive the NT v1 output before launching. Then `sbatch`; it rebuilds from source every run.<br><br>Either way expect ≈17 h and a peak near R11's projection; a large deviation is a signal worth stopping for. | YES |
| **R13** | **P7 DONE 2026-09-16, numeric criteria (i)-(iv) DONE and ALL PASS; plot suite done separately (see below).** P7: snapshot set exact match `[0,1,2,5,10,20,40,43,52,69]`, zero out-of-range against `[-20000,20000]`, zero non-finite across all 10 files; max `\|deltaMvir\|` = 12,432 at snap69, close to the ≈1.5–2×10⁴ plausibility estimate. The four numeric criteria, computed from `model_069.hdf5` (313,766,518 galaxy records): **(i) zero negative `MostBoundID`** — PASS. **(ii) Type-0 count 244,953,607** — PASS, an **exact match** to the upper bound (= the corrected target from §3.2). **(iii) group baryon fraction** in every 0.25 dex bin from 13.00–15.00 Msun (bins with ≥3 centrals) — PASS everywhere, converging to within 0.00001–0.00045 absolute of 0.17 at the high-mass end (physically sensible: less scatter per bin as central count per bin grows). **(iv) Type-0 halo mass function**, 14.00–15.25 Msun bins — PASS, with an **exact match** to the §3.2 corrected expectation (65/41/13/5/1/1) in every one of the six bins, not merely within the ±10% tolerance. Method for (iii): same computation as `models/sage16/plots/figures/baryon_fraction.py` (per-central sum of baryonic components over its whole `UniqueCentralGalaxyID` FoF group, divided by the central's own `Mvir`, mean over centrals per bin) but with 0.25 dex bins to match §3.2's own measurement rather than the plot's 0.1 dex default. Script: `/Volumes/LaCie/staging/shin-uchuu/r13_numeric_checks.py`. The original charge follows as the record. **P7 + P8 — range scan and science checks. Host-agnostic; run wherever R12 ran.** `deltaMvir` scan against `[-20000, 20000]` **after** asserting the job exited 0 and the expected snapshot set `[69, 52, 43, 40, 20, 10, 5, 2, 1, 0]` is complete. Then the full plot suite with the shin-uchuu profile, plus the three-epoch HMF/GSMF check at snapshots 69/52/43 in separate output directories. **The acceptance criteria are numerical, computed from `model_069.hdf5`** — plots may accompany them, they are not the gate: (i) **zero** galaxy records with `MostBoundID < 0`; (ii) **Type-0 galaxy count within `[244,505,058, 244,953,607]`** — the upper bound is the v2 report's own FoF-central halo count, the lower is v1's measured 166,547,771 Type-0 plus 77,957,287 flyby-demoted galaxies, and the interval exists because some halos carry no galaxy record (v1 measured 448,549 such among the demoted); anything outside it stops the run; (iii) mean group baryon fraction within **2 × 10⁻³ absolute** of `BaryonFrac` = 0.17 in every 0.25 dex bin above 10^13.0 Msun holding ≥ 3 centrals; (iv) the Type-0 halo mass function non-empty in every 0.25 dex bin up to at least 10^14.5 Msun, against the corrected expectation of 65 / 41 / 13 / 5 / 1 / 1 centrals in the 14.00–15.25 bins (§3.2), tolerance ±10% per bin on counts above 10. **If R12 ran on NT**, transfer the ≈543 GB output back into a fresh `…/output/sage16-shin-uchuu-v2/`, leaving the v1 directory intact until R14. | YES |
| **R14** | **DONE 2026-09-17.** All five artefacts resolved exactly per the named destinations below, nothing deleted. (1) Scratch dataset copy — confirmed still absent, nothing to retire, per D10. (2) NT dataset copy renamed `snapshot-trees` → `snapshot-trees-v1` — verified **71 files**, an exact match to R1's/D10's recorded count. (3) NT galaxy output: `~/Science/mimic/output/sage16-shin-uchuu/` resolves through a symlink to `/fred/oz004/dcroton/results/sage16-shin-uchuu` — moved to `/fred/oz214/dcroton/shin-uchuu/archive/sage16-shin-uchuu-v1/`, verified **543,411,163,858 bytes (543.4 GB)**. Both `oz004` and `oz214` sit under the same Lustre `/fred` filesystem instance (confirmed via matching `stat -f` fsid on both paths), so the move completed as a metadata-only rename — **0.004 s**, measured with `time mv` — not a data copy. (4) Local galaxy output renamed `sage16-shin-uchuu` → `sage16-shin-uchuu-v1` in place on LaCie — verified **530,675,088 KB (≈543.4 GB)**, matching the NT copy; instant same-volume rename. (5) v1 converter workdir left untouched at `/Volumes/LaCie/convert/shin-uchuu` — measured this session at **≈9.3 GB** (previously recorded as ≈8.7 GB; the small growth is unremarkable and not investigated further). The v2 converter workdir and manifest were not touched. The original charge follows as the record. **Retire v1 — only after R13's acceptance criteria pass, with capacity proved before anything moves.** Nothing is deleted; the project convention is archive-only. Five artefacts, with their destinations named here rather than left to the executing agent:<br>1. **Scratch dataset copy** — **does not exist; nothing to retire.** Superseded by D10: the local v1 dataset was deleted at R1 rather than archived to Scratch, so NT (item 2) is v1's sole copy and item 2 is the only dataset artefact to retire. Do not go looking for a Scratch archive — its absence is intended, not a missing step. *(If the optional sorted-state archive was taken mid-R9, it lives here too and this copy will have been released; the R9 decision point records which.)*<br>2. **NT dataset copy** (2.25 TB, `/fred/oz214/dcroton/shin-uchuu/snapshot-trees/`) — rename in place to `snapshot-trees-v1/`. `/fred` has 11 PB free and no `oz214` quota, so capacity is not a constraint; confirm with `df -h /fred` anyway and record it.<br>3. **NT galaxy output** (≈543 GB, `~/Science/mimic/output/sage16-shin-uchuu/`) — move to `/fred/oz214/dcroton/shin-uchuu/archive/sage16-shin-uchuu-v1/`.<br>4. **Local galaxy output** (543.4 GB, `/Volumes/LaCie/data/shin-uchuu/output/sage16-shin-uchuu/`) — rename in place to `sage16-shin-uchuu-v1/`; same volume, so it is a rename, not a copy, and costs no space.<br>5. **The v1 converter workdir** (`/Volumes/LaCie/convert/shin-uchuu`, 8.7 GB) — **keep as is**. It holds the manifest and the two `.npy` tables and is the provenance record for what v1 was.<br><br>For each move: `df` the destination first, then verify **file count and total bytes** at the destination before touching the source. **Do not retire the v2 converter workdir or its manifest** — they are v2's provenance. | No |
| **R15** | **DONE 2026-09-17.** On NT: the (empty) `working/` directory and the two superseded Slurm log files — found in NT's home directory at `/home/dcroton/Science/mimic/sage16-shin-uchuu-16267967.{out,err}`, not under `working/` itself — both moved into `/fred/oz214/dcroton/shin-uchuu/archive/`, leaving `snapshot-trees-v2/`, the v2 output and the R14 v1 artefacts untouched, exactly as scoped. Locally: `/Volumes/LaCie/staging/shin-uchuu/` (7.0 GB, holding both v1's and v2's driver records together) moved whole to `archive/shin-uchuu-run/staging/` (this repository's gitignored `archive/`, on the boot volume, reusing an empty placeholder directory already reserved for it). **This document's own Definition of Done (0–11) is now confirmed in full** — see that section below — and the remediation is closed. **Next: F1 (Uchuu-family particle mass) and F2 (merge `feature/ctrees-snapshot-reader` → `main`)** — deliberately not started in this session; see `MIMIC-DEVELOPMENT-PATHWAY.md`'s "NEXT TASK" banner. A related but out-of-scope finding surfaced during this closeout's plot regeneration: two `mimic-plot` evolution figures (`smf_evolution`, `stellar_mass_density_evolution`, in all three model packages that carry the pattern) were gating on a single sample snapshot rather than scanning all snapshots, producing a false "all zero" skip when that sample happened to be the earliest, pre-star-formation epoch. Fixed in the same session (`check_field_has_values_any_snapshot()` in `plot/mimic-plot/output_utils.py`); unrelated to the flyby defect and not part of this addendum's scope, noted here only because it was found while closing this out. The original charge follows as the record. **Close out.** Cleanup here means **archive, not delete** — the project convention admits no exceptions, and a reader who takes "cleanup" literally will remove evidence. On NT: `/fred/oz214/dcroton/shin-uchuu/working/` and the Slurm `.out`/`.err` files from superseded job attempts move to `/fred/oz214/dcroton/shin-uchuu/archive/`. **Not** `snapshot-trees-v2/`, **not** the v2 output, **not** `/fred/oz214/simulations/uchuu/shinuchuu/` (read-only source, not ours), and **not** the archived v1 artefacts from R14. Locally: `/Volumes/LaCie/staging/shin-uchuu/` moves to `archive/`. **Note it holds both runs' records** — v1's frozen batch plan, logs, `state.json` and `run_p1_p2.py` alongside v2's — so archive it whole, and not until R9 has finished with it. The v1 and v2 converter workdirs both stay where they are. Then confirm this document's Definition of Done — **not** the conversion plan's, whose items are v1 evidence — and proceed to F1 (Uchuu-family particle mass) and F2 (merge `feature/ctrees-snapshot-reader` → `main`). | No |
...
Its corrections and additions are folded into the text above: the package exposure is four packages rather than two; the `upid` assignment is at `ctrees_utils.c:408`, not `:409`; five further physics consequences (§4.2); the complete file-level blast radius including the committed `micro-uchuu-horizontal` fixtures (§4.3); the missing multi-FoF regression test (R3); the minimum retention set for `micro-uchuu-ascii` (D2); the v1 compatibility policy question (D3); and the sharper statement of why the in-place repair does not need index rewriting (D4).
...
2. `format_version = 2` is stamped by the producer, **required** by the consumer with no legacy-read path, and specified in `HORIZONTAL-HDF5-FORMAT.md`; the archived v1 dataset is rejected with a message naming the file and the version found.
...
10. All five v1 artefacts are dealt with per R14's named destinations — Scratch dataset (already archived at R1), NT dataset, NT galaxy output, local galaxy output, v1 converter workdir (kept) — with capacity proved and file counts and byte totals verified at each destination before any source was touched. **Nothing was deleted.** The v2 workdir and manifest are retained. **SATISFIED 2026-09-17** — see R14's DONE record above for the verified byte counts and the confirmation that both galaxy-output moves (NT and local) matched at 543.4 GB.
11. This addendum, the conversion plan and the development pathway all record the outcome, and the conversion plan's own Definition of Done is explicitly marked v1-historical. **SATISFIED 2026-09-17** — this document is closed (see the status line at top); `MIMIC-DEVELOPMENT-PATHWAY.md` Step 1's status and its "NEXT TASK" banner are updated to point at F1/F2; `SHIN-UCHUU-CONVERSION-PLAN.md`'s own Definition of Done was already marked v1-historical by this addendum's "Precedence" note above, unchanged this session. The root `HANDOFF.md` that tracked R8–R15 is archived to `archive/handoffs/HANDOFF-2026-09-17-flyby-defect-remediation-complete.md`; every fact it held is captured in this document (the R1–R15 table above) or in the pathway document's completed-work log.

**All 12 items (0–11) are satisfied as of 2026-09-17. The Shin-Uchuu flyby-defect remediation is closed.**
