...
- **Cross-format identity depends on more than RNG.** `UniqueGalaxyID` is encoded from `(forestnr_global, halonr)`, where `forestnr_global` is the run-scoped forest index over the selected input set. For Phase 5 cross-format identity the snapshot driver must reproduce the same per-galaxy identity from the same global forest ordering, so treat the identity scheme (not just stochastic seeding) as a first-class cross-format-identity contract.
