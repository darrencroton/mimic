...
>
> **`report.py` will not find it for you.** `recommended_multiplier()` (`scripts/convert/report.py:34-46`) searches only powers of ten, so from 10⁹ it reaches 10¹¹, finds `10¹¹ × (166,547,771 + 1) > INT64_MAX`, and raises `no valid identity multiplier` — aborting the production conversion report even though the window above is non-empty and 4.3 × 10¹⁰ wide. Confirm the real `max_halo_rank_in_forest` from the report's own measured counts and set the multiplier by hand, or widen that search first.
...
**Decision (owner, 2026-08-26): do NOT implement the compact previous-slab projection as a precondition.** It saves 80 B/halo = **28.4 GB against a 35.4 GB overshoot** — it does not close the gap — and the trigger's 15% headroom assumption does not match this host: measured committed memory outside Mimic is **≈37.7 GB** (31.0 GB anonymous + 6.7 GB wired; the 170.8 GB of file-backed pages is evictable cache), so a lean box leaves Mimic **≈540 GB** and 470.4 GB sits at ~87% with ~70 GB spare.
...
> - **NEW item 8 — `recommended_multiplier()` searches only powers of ten.** It is small, self-contained, and a **hard blocker on the production conversion report**, so it can and should be landed early and independently of the rest of this pass. See "Simulation package changes required" above for the full finding and the feasible window.
>
