...
| **P6** | **Production `sage16` run**, against the **P3b-verified** dataset. **Relocated to NT 2026-09-04 — see the callout above.** Runs on an NT large-memory node (≈768 GB–1.03 TB), reading the dataset **P9 copies there first** (P9 moves ahead of P6 for this run). Build and run commands below describe the Mac-Studio-local case; the NT equivalent (paths, partition, job submission) is next session's work. `MODEL`/`SIMULATION` are compile-time (C11). Run detached through a wrapper that **records the true exit code** — P7 must not start on a failed run. Projected peak **≈639–697 GB measured** (superseding the ≈476.6 GB pre-measurement projection — see P5 above) | **≈8–16 h (estimate; the rehearsal wall clock is not committed)** | ⚠️⚠️ **the maximum-memory task of the whole sequence** | YES |
...
> **⛔ v1 HISTORICAL EVIDENCE, NOT CURRENT COMPLETION (2026-09-09).** Every item below was satisfied by the v1 conversion and the v1 `sage16` run, and every one of them is now void for acceptance purposes: the dataset those runs produced is structurally wrong at z=0 (see the "P8 FAILED THE SCIENCE CHECK" callout above). The live acceptance criteria are the eleven items in [`SHIN-UCHUU-FLYBY-DEFECT-ADDENDUM.md`](SHIN-UCHUU-FLYBY-DEFECT-ADDENDUM.md) → "Definition Of Done". This list is kept because the *measurements* behind each item — wall clock, peak RSS, ranges, counts — remain valid evidence about the pipeline, and because the v2 run has to satisfy the same items again.
