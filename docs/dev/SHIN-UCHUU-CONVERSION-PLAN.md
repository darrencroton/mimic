...
>
> **Decision (owner, 2026-09-04): relocate P6 to the Ngarrgu Tindebeek (NT) supercomputer, on a large-memory node.** `MIMIC-DISTRIBUTED-SNAPSHOT-PLAN.md` was checked as a candidate fix and ruled out — it targets cluster deployment via MPI domain decomposition, not a single low-memory workstation, and is itself unbuilt (gated behind a second unbuilt plan, and today's driver rejects `mpirun` outright for snapshot-ordered runs per C10). Reading NT's `slurm.conf` (read-only, from the login host) found node classes at ≈1.03 TB (`gina[301-305]`, `dave[301-311]`), ≈770 GB (`gina[18]`, `dave[148-149]`), and ≈768 GB (`bryan[5-8]`) — any of which clears even the unmitigated ≈697 GB projection with Mimic's existing serial-only snapshot driver, unmodified. **Access is not yet confirmed** — the partitions read `State=DOWN` in that config and several carry account-allowlists (`hpcadmin`/`oz987`/`oz500`) that do not name this project's `oz214`; resolving that is the owner's task for the next session, not something to infer here.
>
...
**P7 does not start until that file reads `0`.** A failed run leaves the partitions it had already finished **intact and complete on disk**: `bye()` unlinks only the partition in flight, "so a crash never leaves partial output files behind **and never deletes completed ones**" (`src/core/tree_driver.c:42-46`, `tree_driver_remove_incomplete_outputs()` at `:59-65`). **For a snapshot-ordered run — which P6 is — the equivalent lives in `src/core/snapshot_driver.c:81-96`**: two independently-armed `bye()` slots, INFLIGHT for the partition being written (released the moment that file closes cleanly, so a completed snapshot's output survives a later failure) and MASTER for the run's master file. Every surviving file is therefore individually well-formed, and no per-file check distinguishes that set from a complete run's — which is why P7 asserts the exact expected snapshot set as well as the exit code. Diagnose from `run.log` and re-run: snapshot-ordered runs are not resumable (C10), so a failed run is a full restart, not a continuation.
...
4. ✅ **DONE.** `simulations/shin-uchuu/` package registered and building clean with the snapshot reader
