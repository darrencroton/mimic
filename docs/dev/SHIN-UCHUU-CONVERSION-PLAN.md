...
**Status:** Converter built and micro-Uchuu-validated 2026-07-24 — the external converter described here exists under `scripts/convert/` and passed its micro-Uchuu acceptance gate (full pipeline over the real 22,580,924-halo / 50-snapshot ASCII data; producer validation battery + a seven-check cross-check against a `halos-only` reference run, topology-order proof fully discharged, zero unexplained mismatches). That gate was re-run end to end on 2026-08-03 on a fully regenerated micro-Uchuu dataset, now placed at `/Volumes/Internal/data/uchuu/micro-uchuu/micro-uchuu-snapshot/` for horizontal-reader development: same three totals, producer battery 15/15, cross-check green including `topology-chains`. **P1–P3 of the production conversion ran to completion 2026-08-29 to 2026-09-03**, and **P3b–P5 ran to completion 2026-09-04** — see "The Production Execution Sequence" → "Production run complete" and "P3b, P4 done and measured" for the measured totals. **P6's first NT attempt (2026-09-05) crashed on a core dispatch bug, unrelated to P5's memory analysis; the bug is fixed and committed (`93292fda`).** **P6's retry (job `16267967`, 2026-09-07) succeeded end to end** — see "P6 attempt 2 succeeded" below. P7's post-run `deltaMvir` scan ran 2026-09-08. All previously open design decisions resolved (joint plan review 2026-07-02, decisions D1–D12; review record archived at `archive/dev-plans/dual-driver-plan-review.md`). Earlier revision reviewed twice by Codex gpt-5.5 (2026-06-27).
...
**Context:** This plan is one sequence with `MIMIC-DUAL-DRIVER-PLAN.md`. The converter is **not** blocked on the horizontal driver: it is blocked only on the frozen format contract, and it is built and validated first, against micro-Uchuu ASCII, using the existing vertical `read_ctrees_ascii.c` reader as the reference — zero new Mimic code required. The full 11.61 TB Shin-Uchuu conversion runs exactly once, after the dual-driver Phase 5 identity gate is green (**green 2026-08-12**). Mimic itself performs no internal conversion.

**Re-conversion notice (2026-09-10).** This plan's P-sequence (P1–P9) was executed once, in full, producing the v1 Shin-Uchuu dataset — and that dataset was then found scientifically defective at z=0 (the `fix_flybys` topology defect). `fix_flybys` has since been deleted from Mimic and the horizontal format bumped to `format_version = 2`. The re-conversion follows [`SHIN-UCHUU-FLYBY-DEFECT-ADDENDUM.md`](SHIN-UCHUU-FLYBY-DEFECT-ADDENDUM.md)'s R1–R15 sequence, not a re-run of P1–P9. This document remains authoritative for the measured source data, the conversion algorithm, the storage/memory envelopes, flag semantics and footguns, and the operational detail of every stage — the addendum's route is deliberately the same route described here — but its own P-sequence outcome status (including the Definition of Done below) is v1-historical, not current completion. Read the addendum first; see also the "P8 FAILED THE SCIENCE CHECK" record further down for the full defect account.
...
1. **Percolation super-forest**: forest `26551468179` contains 104,845,278 tree roots — 33% of all trees — almost certainly a ctrees linking artifact. The vertical driver must load a forest as a single in-memory unit; it cannot. This also rules out the uchuutools forests-HDF5 packaging.
...
Both problems are structural consequences of forest-ordered processing and both disappear in the horizontal driver, which processes one snapshot's halo population at a time.
...
## Target Format: Horizontal HDF5

One HDF5 file per snapshot, named `snapshot_000.h5` through `snapshot_069.h5` (per-snapshot files are decided, not open: partial recovery, per-snapshot parallelism, and the driver's access pattern all favour them). All topology links are snapshot-local integer indices (no global IDs). Scalar metadata lives in HDF5 **attributes** on the `/header` group. **Field names and types on disk must match what `simulations/shin-uchuu/halo_properties.yaml` declares** — except `ForestIndex` and `HaloRankInForest`, which are exempt (see the package section below and `HORIZONTAL-HDF5-FORMAT.md` errata 2026-08-11) — so the generated `RawHalo`/accessors consume the file directly; the names below already match the existing `micro-uchuu-ascii` bridge contract (`M_Crit200`→`HaloMass`, `Len`, `SnapNum`, `MostBoundID`, spin conventions). The contract is **frozen** at [`docs/dev/HORIZONTAL-HDF5-FORMAT.md`](HORIZONTAL-HDF5-FORMAT.md) (originally `format_version = 1`, 2026-07-18; now **`format_version = 2`**, 2026-09-10, per [`SHIN-UCHUU-FLYBY-DEFECT-ADDENDUM.md`](SHIN-UCHUU-FLYBY-DEFECT-ADDENDUM.md) — v1 is rejected outright), which is now authoritative; this section remains as the working draft it was promoted from — if they ever disagree, the spec wins.
...
snapshot_NNN.h5
  ...
  /halos/
    ...
    ForestIndex           int64[N]    dense run-scoped forest number; identity component
                                      ...
                                      depends on it)
    HaloRankInForest      int64[N]    per-forest halo index in reference vertical-driver order
...
**Decided 2026-08-26, and constrained by permissions rather than preference. Superseded 2026-09-04 on the "must stay on the Mac Studio" claim — see below.** The converted dataset is produced on the Mac Studio; at the time this was written, the plan was for the production `sage16` run to stay there too (`tooarrana1`'s 251 GB was never a candidate host regardless — it is a streaming host, not a compute node). **That plan changed once P5's real re-projection landed at ≈639–697 GB against the Mac Studio's ~540 GB available** (`SHIN-UCHUU-CONVERSION-PLAN.md` → "P3b, P4 done and measured 2026-09-04"): P6 is relocating to an NT large-memory node instead, which means the copy to `/fred/oz214/dcroton/shin-uchuu/snapshot-trees/` below is no longer a "post-P6" archival nicety — it is how the dataset reaches the host that runs P6, and it now happens **before** P6, not after (P9 moved ahead of P6 for this run; see the P5 callout and the P9 table row above). The path table and the permission constraint below are unaffected by this change — only the "stays local, archive later" framing is retired.
...
1. Fetch `.dat` files in batches with plain `rsync -aL` (the 93.5 GB largest file is a single-batch item). **Do not pass `--checksum`** — measured on the production transfer 2026-08-29: a `--dry-run` over a real 685-file batch list, with `--checksum`, was still running with the remote server pegged near 100% CPU after 3.5+ minutes when killed; the identical dry-run without it completed in 0.6 s. It scales with file-list size, not data volume, and is unusable at this scale with this openrsync/GNU-rsync combination. Dropping it costs nothing here: this is a one-time pull of read-only source data (no local copy to reconcile against), and integrity is already covered three other ways — SSH's own authenticated transport, this step's own post-transfer per-file size check against the real remote inventory, and the converter's independent per-file md5 plus row pre-count during `scatter` (which reads and parses every file regardless).
...
For each snap_N+1 halo D (by local index):
    ...
    Contract item 1 (docs/dev/HORIZONTAL-HDF5-FORMAT.md) has been corrected to describe
...
HaloRankInForest[i]: the within-forest index in REFERENCE vertical-driver order —
...
well-defined for every forest, including the super-forest the vertical driver
...
| `snapshot_NNN.h5` `/halos` | `HORIZONTAL-HDF5-FORMAT.md`, 16 datasets | **100 exactly** |
...
- **Home:** `scripts/convert/` in the Mimic repo, with converter tests alongside. It is a long-lived, versioned science tool whose output contract co-evolves with the horizontal reader — not a one-off script.
...
Exact match to `apply_ctrees_value_conventions()` (`src/io/vertical/read_ctrees_ascii.c:96-106`):
```python
...
**Two packages, not one — recorded 2026-08-25.** The rehearsal runs the cross-format identity gate, which by construction needs a *vertical* run over the same data as the horizontal run. That mirrors the micro-Uchuu pair (`micro-uchuu-ascii` + `micro-uchuu-horizontal`) and this plan previously named only the horizontal half.

- **`simulations/shin-uchuu-ascii/`** — `tree_type: consistent_trees_ascii`, `snapshots/` pointing at the **subset** ASCII directory. This package is only ever usable on a subset: the full dataset cannot be processed vertical at all, which is this plan's entire premise. Its `halo_properties.yaml` mirrors `micro-uchuu-ascii`'s ctrees bridge contract.
- **`simulations/shin-uchuu/`** — the horizontal package below. Its `snapshots/` symlink points at the **subset** HDF5 during the rehearsal and is re-pointed at the production dataset after the production conversion.
...
>
> **`report.py` now finds it for you — CLOSED 2026-08-26 (Task 11 item 8).** It previously searched only powers of ten, so from 10⁹ it reached 10¹¹, found `10¹¹ × (166,547,771 + 1) > INT64_MAX`, and raised `no valid identity multiplier` — aborting the production conversion report even though the window above is non-empty and 4.3 × 10¹⁰ wide. `recommended_multiplier()` now derives the window from the same two conditions the reader enforces at run time in `horizontal_identity_bounds_valid()` (`src/io/horizontal/interface.c:100-129`) — `multiplier > max_halo_rank_in_forest` and `n_forests_total <= INT64_MAX/multiplier - 1` — and searches powers of ten first, then the 1/2/5 ladder that fills the gaps between them. On the production figures it returns exactly the **2 × 10¹⁰** prescribed above, and the report additionally records the whole window as `identity_multiplier_window` so the value can be confirmed rather than recomputed by hand. Still confirm the real `max_halo_rank_in_forest` against the production report's own measured counts.

A new `simulations/shin-uchuu/` package:
- `simulation_info.yaml`: 140 Mpc/h box, confirmed particle mass, `tree_type: horizontal_hdf5`, `processing_order: horizontal` in run files, 70-snapshot list, and the identity multiplier from the conversion report (D9)
- `halo_properties.yaml`: ctrees bridge contract mirroring `micro-uchuu-ascii` (`M_Crit200` → `HaloMass`, `Len`, `SnapNum`, `Pos` range `[0.0, 140.0]`). **Do not declare `ForestIndex` or `HaloRankInForest`** — corrected 2026-08-12. They are horizontal-format identity metadata rather than catalog halo properties, and are exempt from the declaration rule (`HORIZONTAL-HDF5-FORMAT.md` errata 2026-08-11): the reader consumes both directly by dataset name into `struct SnapshotSlab`'s own `forest_index`/`halo_rank_in_forest` arrays. Declaring them is unnecessary rather than forbidden, but the working exemplar `simulations/micro-uchuu-horizontal/halo_properties.yaml` omits both, and mirroring it is the safe course
- `shin-uchuu.a_list`: from Phase 1
- `snapshots/`: symlink to the 70 HDF5 files

**Run constraints for horizontal runs** (added 2026-08-12; enforced at config time in `src/core/read_parameter_file.c:1475-1488`, so a production run that violates one aborts at startup rather than part way through):
...
2. **The converter refuses a mismatched index.** `validate_root_coverage()` (`scatter.py:192-216`) enforces **one-to-one** coverage between observed `#tree` roots and `forests.list` — surplus listed roots abort just as loudly as missing ones. A subset therefore needs its **own** `forests.list`, and the vertical reference run needs its own `locations.dat` to match.
...
5. **The reader requires file IDs to be contiguous from 0 *and* the file count to be a perfect cube.** `read_locations()` asserts `max_fileid + 1 == numfiles` and then `round(cbrt(numfiles))³ == numfiles` (`src/io/vertical/ctrees/ctrees_utils.c:236-246`). **2,744 = 14³** — the full dataset satisfies this exactly, which means the subset must too. An earlier draft of this design claimed sparse original `FileID`s were safe because the fd table grows on demand; **that was wrong** — the table does grow, but both assertions fire afterwards. See the file-coverage rule below.
6. **A forest's reader memory has a floor set by its *tree count*, independent of its halo count.** `load_unit_ctrees_ascii()` preallocates `1000 × ntrees` records in **both** `halo_data` and `additional_info` before reading anything (`src/io/vertical/read_ctrees_ascii.c:647-655`). Measured exactly: `sizeof(struct halo_data)` = 104 B and `sizeof(struct additional_info)` = 48 B, so the preallocation is **152,000 B per tree**. At the measured 72.8 halos/tree this over-allocates by ~13.7×, and it dominates — but the arrays still **grow** if a forest's actual halo count exceeds that capacity (`src/io/vertical/ctrees/parse_ctrees.h:365-384`), so tree count sets a floor rather than a ceiling. That is why the gate below has two halves. The super-forest would demand 104,845,278 × 152,000 B ≈ **15.9 TB**, which settles its exclusion beyond argument — but it also means a forest with many *small* trees can be intractable while its halo estimate looks modest.
...
At a 5 × 10⁶-tree target spread over 2,744 files this is satisfied ~1,800× over *on average* — but average is not proof, and a coverage hole would surface only at Task 6's vertical reference run, after extraction and transfer are paid for. **Make it an explicit acceptance assertion.**
...
- **High-mass supplement.** The top `K` tractable forests by **measured maximum root `Mvir`** from Step 2b (suggest K = 200) — not by `total_bytes`, which is only a proxy. These are the most massive systems **in the byte-selected candidate pool** — the best available proxy for "most massive outside the super-forest", with its recovery fraction measured on micro-Uchuu rather than assumed — and they exercise the largest-forest vertical-driver memory path.
...
| Transfer stalls / partial batches | Medium | plain `rsync -aL` (not `--checksum` — unusable at this file-list scale, see "Transfer" above), per-file resume manifest, consumptive deletes keep disk bounded |
...
3. **UniqueGalaxyID** — horizontal runs reproduce vertical-driver IDs exactly via dense `ForestIndex` + reference-ordered `HaloRankInForest` and the per-simulation identity multiplier (D9).
...
3. ~~Dual-driver Phase 4b: horizontal reader against the micro-Uchuu fixtures~~ — **done 2026-08-04** (`MIMIC-SNAPSHOT-READER-PLAN.md`); the format this plan emits is now readable and validated by Mimic
4. Dual-driver Phase 5: horizontal driver + cross-format identity gate on micro-Uchuu — **done 2026-08-12** (gate green: both models, both timestep schemes, per-ID bitwise)
...
- **A production-scale topology cross-check remains impossible in principle**, not merely expensive: its reference side is a vertical `halos-only` run over the same data, and the ctrees reader preallocates 152,000 B per tree for a whole forest before reading a halo, so the super-forest's 104,845,278 tree roots alone project to ≈15.9 TB of reader preallocation. **The binding cross-check gate is micro-Uchuu**, as the Definition of Done already says.
...
>
> P1 (transfer) and P2 (production conversion) ran to completion 2026-08-29 to 2026-09-03, and P3 (the identity multiplier) is set and committed (`5506ef88`). The measured totals from the production `conversion_report.json`, independently re-verified against the raw report rather than taken from any summary — `validation_passed: true`, all 70 snapshots present with non-zero halo counts, `outputs_dir` matches the intended path exactly: **22,503,649,037 halos**, `n_forests_total = 166,547,771`, `max_halo_rank_in_forest = 12,646,607,900`. The emitted dataset measures **2.2520 TB** — 2,250,651,788,416 B across the 70 `snapshot_NNN.h5`, i.e. **100.01 B/halo**, plus 1,332,774,956 B for `forests.h5` — within 2.5% of the ≈2.31 TB / 100.7 B/halo pre-run projection recorded above under "Pass complete", which was therefore sound. **Corrected 2026-09-09**: this line previously read "**2.0 TB** (`du -sh`), ≈89 B/halo" and claimed to supersede that projection. macOS `du -sh` reports **TiB**, not TB — 2.0 TiB is 2.20 TB — so the "89 B/halo" figure was a unit error that under-budgets output storage by ≈11%. Plan against 100 B/halo. P1+P2 total wall clock was ≈138.3 hours (5.76 days) against this document's own "2–4 days" estimate for P2 alone: the batch-mode transfer cycle being strictly sequential rather than overlapped, and several stages (`finalize` especially) running well past naive extrapolation from rehearsal-scale figures, both contributed. **Two operational findings from the production run are now folded into "Transfer" and P2 Step 1 above** (dropping `rsync --checksum`, and the 7.0 TB cold-start-vs-1.0 TB between-batch floor distinction) — done 2026-09-04, migrated from that session's gitignored `HANDOFF.md` before it was archived to `archive/handoffs/`. **P3b was not done in this session** — see the callout immediately below for what happened next.
...
>
> **P5 (memory re-projection) does not clear, and blocks P6 on the Mac Studio as originally specified.** Using `N = max(entry["rows"] for entry in report["per_snapshot"].values())` from the real production `conversion_report.json`: the largest slab is **snapshot 34 at 519,342,987 halos** — not the pre-measurement projection of ≈3.546 × 10⁸, but **46.5% higher**, on a broader/taller plateau (snapshots 31–40 all in the 494–519M range) than the rehearsal predicted. Applying this document's own fit, **peak ≈ 1.221 GB + 1,340.6 B/halo × N ≈ 697 GB** unmitigated, and **≈639 GB** even with the deferred compact-previous-slab mitigation applied (`peak_mitigated ≈ 1.221 GB + 1,228.6 B/halo × N`) — both well above the documented ≈589 GB ceiling above which "the post-mitigation peak is still over the ~540 GB available," and both far above the ~540 GB this host actually has.
>
> **Decision (owner, 2026-09-04): relocate P6 to the Ngarrgu Tindebeek (NT) supercomputer, on a large-memory node.** `MIMIC-DISTRIBUTED-SNAPSHOT-PLAN.md` was checked as a candidate fix and ruled out — it targets cluster deployment via MPI domain decomposition, not a single low-memory workstation, and is itself unbuilt (gated behind a second unbuilt plan, and today's driver rejects `mpirun` outright for horizontal runs per C10). Reading NT's `slurm.conf` (read-only, from the login host) found node classes at ≈1.03 TB (`gina[301-305]`, `dave[301-311]`), ≈770 GB (`gina[18]`, `dave[148-149]`), and ≈768 GB (`bryan[5-8]`) — any of which clears even the unmitigated ≈697 GB projection with Mimic's existing serial-only horizontal driver, unmodified. **Access is not yet confirmed** — the partitions read `State=DOWN` in that config and several carry account-allowlists (`hpcadmin`/`oz987`/`oz500`) that do not name this project's `oz214`; resolving that is the owner's task for the next session, not something to infer here.
>
> **This run is explicitly not a one-off.** It is a scientific verification that the conversion is sound, and a stepping stone: `sage16` and future/new model packages will run against Shin-Uchuu for years, a local-machine run is wanted eventually, and larger simulations with the same lumpy-slab problem are expected. Relying on ever-larger single hosts is therefore not accepted as the durable fix — a generic, reusable memory bound is, tracked separately as a concept note: [`MIMIC-CHUNKED-SLAB-STREAMING-PLAN.md`](MIMIC-CHUNKED-SLAB-STREAMING-PLAN.md). NT unblocks **this** run; that note is what unblocks every run after it without depending on which host is available.
>
> **Consequence for the step order below: P9 moves before P6.** P6 will read from NT-local storage, not `/Volumes/LaCie`, so the production dataset must be copied to NT first — which is exactly P9's already-planned action (`rsync` to `/fred/oz214/dcroton/shin-uchuu/snapshot-trees/`), just performed before P6 instead of after. The table below is not renumbered (P9's identity and acceptance criteria are unchanged), but **execute it ahead of P6** for this run. **Next session's task, not decided here:** confirm NT account/partition access, transfer the 2.2520 TB production dataset via `rsync -aL` (reverse direction from P1, same tool and gotchas), set up a `mimic` checkout and build on NT (Mimic is confirmed to run there, but directories, environment and the package's `snapshots` symlink all need setting up fresh — the Mac Studio's P3b re-point is local-machine state only and has no bearing on the NT checkout), then run P6 there on a confirmed-accessible large-memory node.

> ### P6 attempt 1 crashed and fixed 2026-09-05 — not a memory problem, retry cleared
>
> **P9 and P6's setup both completed successfully**: the dataset transfer, NT checkout, and build all worked, access to an `oz214`-accessible ≈1 TB-class node was not a blocker (`ops/nt/sage16_shin-uchuu.sbatch`, job `16193838`), and `validate-modules` passed before the run started. **The run itself crashed at snapshot 35/70 (50%), ≈5h57m wall clock in, on `FATAL_ERROR`** — but the job report's peak RSS was **472.3 GB**, comfortably under the node's budget and consistent with P5's own re-projection. **The memory wall P5 identified is not what stopped this run**; P5's analysis and the NT relocation both held.
>
> **Actual cause: a fixed-size core dispatch buffer, unrelated to memory.** `sage_resolve_mergers_and_disruption` (the merger/disruption resolver) emitted more merger events in one FoF workspace than the core module dispatcher's phase-local event buffer could hold — a fixed 4096-slot static array (`MAX_PHASE_EVENTS`, `src/core/module_registry.c`) sized against `mini-millennium`'s much smaller/sparser halo groups (~460 events/FoF-snapshot measured, `OPTIMISATION-SPECTRUM.md` item 38) and never stress-tested against Shin-Uchuu's much larger ones. The exact log lines: `ERROR: Phase event buffer overflow: emitted 4096 events (max 4096)` → `ERROR: Failed to emit immediate merger event (source=8548, target=22, ratio=0.000001)` → `FATAL: Module 'sage_resolve_mergers_and_disruption' failed (substep 9)`. Since horizontal runs have no `--skip` resume (C10) and physics is deterministic given a halo's own data (VISION.md Principle 4), a bare resubmit on the unfixed binary would have hit the identical abort at the identical point — this was not survivable by retrying alone.
>
> **Fixed the same day, committed at `93292fda`.** The fixed array is replaced with a heap grow-to-high-water buffer following the exact precedent already established for this class of problem — `ensure_fof_workspace_capacity()` (`src/core/build_model.c`), same `HALO_ARRAY_GROWTH_FACTOR`/`MIN_HALO_ARRAY_GROWTH`/`MAX_HALO_ARRAY_SIZE` arithmetic, same fail-fast ceiling — so the fix is generic across every simulation package and every current or future event producer, not a Shin-Uchuu-specific number, and adds no new user-facing tunable. `module_emit_event()`'s public contract, event dispatch order, and subscription routing are all unchanged; no physics module was touched. Implemented by a Claude Opus 5 (high-effort) delegate under a frozen contract; independently reviewed by Codex gpt-5.6-sol (high effort, read-only) with **no P0/P1 findings** — one stale doc comment (`module_interface.h`'s failure-cases description) and one incomplete-evidence gap (`check-docs`/scientific tier not yet run at review time), both closed by the Developer directly. Full validation: clean build under `-Wall -Wextra -Wshadow -Wformat-security -Wundef`, `validate-modules`, `check-generated`, `check-docs`, `check-format` all green; full unit tier 458/458, full integration tier 39/39 (including `tests/integration/test_event_routing.py` and `test_processing_modes.py` specifically), scientific tier green; new regression test `tests/unit/test_event_buffer_growth.c` proves a single `execute_phase()` call emitting 5000 events (past the old cap) completes and delivers every event, with zero memory leaks.
>
> **P6 is cleared to retry.** Pull the NT checkout to `93292fda` or later, archive the crashed attempt's `sage16-shin-uchuu-16193838.{out,err}` (never delete — this repository's file-organisation convention), then resubmit `ops/nt/sage16_shin-uchuu.sbatch` unchanged — it rebuilds from source every run (`make clean && make`), so the fix is picked up automatically. No other step in this sequence is affected; P7–P9's acceptance criteria are unchanged.

> ### P6 attempt 2 succeeded 2026-09-07 — the run is complete, P7 passed 2026-09-08
>
> **Job `16267967`, resubmitted against the fixed checkout, ran to completion.** Slurm's own job report: `COMPLETED`, peak RSS **472.3 GB** against the node's 878.9 GB (53.7% utilisation), wall clock **17h04m43s** (0-17:04:43) — the first committed measurement of this quantity, well inside the "≈8–16 h, estimate" this document previously carried and inside the ≈639–697 GB P5 projected. Mimic's own end-of-run report (`print_run_memory_profile()`) gives a second, slightly higher figure — **peak process RSS 507.384 GB** — plus the terms behind it: output buffer capacity `C` = 545,310,136 records (95.975 GB), output population `P` = 516,296,205 (`P`/N ≈ 0.9941 against the largest slab, consistent with the ≈0.997 figure measured at rehearsal scale), galaxy pool high-water `G` = 542,590,467 (95.496 GB), pool allocated across 138 chunks with **0.5% chunk slack**. (The two peak-RSS figures — Slurm's cgroup accounting vs Mimic's own `getrusage` sampling — are expected to differ somewhat; both are recorded here rather than reconciled, and both sit comfortably inside the node's budget.) The tree-processing portion of the run (100% at 70/70 snapshots) completed in 15:59:57 per its own progress bar; the remainder of the 17h04m43s wall clock is module loading, the clean build, `validate-modules`, and the plotting step. All ten requested output snapshots were emitted (`model_000.hdf5` … `model_069.hdf5` matching `[69, 52, 43, 40, 20, 10, 5, 2, 1, 0]` exactly), totalling **≈543.4 GB** — confirmed both from the file listing and from the job report's own `/fred` write figure (555.3 GB, the small remainder being metadata/plots). The sbatch script's own final step (a bare `mimic-plot.py --param-file=...` with no snapshot/output-dir flags) ran too, producing its **default 22 figures** (18 snapshot-tier at the last requested snapshot, z=0, plus 4 evolution plots spanning all ten), zero skipped — this is the plot set now archived locally at `archive/plots/`.
>
> **P7 (`deltaMvir` post-run range scan) ran 2026-09-08, directly on NT against the production output (no transfer needed for this scan).** Snapshot set assertion passed (found exactly `[0, 1, 2, 5, 10, 20, 40, 43, 52, 69]`, no more, no fewer). Scanned **2,058,069,519** galaxy records across the ten files. Result: **min/max = [−12363.600, +12432.001]**, **zero out-of-range, zero non-finite** — **ACCEPT** against the declared `[-20000, 20000]` bound. Per-snapshot extrema grow with cosmic time as expected (z=0's `model_069.hdf5` carries the widest range, ±~1.24×10⁴); the earlier "1.5–2×10⁴, at or near ±20000" plausibility estimate (§6 item 9) was in the right order of magnitude but ran a little high — the real production extremum sits at **62% of the bound**, a real margin (not a violation) but noticeably thinner than the 254×/97× margins the rehearsal-subset figures showed, exactly as that estimate anticipated. Definition-of-Done item 5 ("Mimic runs sage16 on Shin-Uchuu end-to-end: no assertion failures, no broken links, no memory errors") is satisfied.
>
> **P8 is only partly done.** The default plotting step above only exercises the *last* requested snapshot (69, z=0) for the 18 snapshot-tier figures — `mimic-plot.py` with neither `--snapshot` nor `--all-snapshots` defaults `snapshot = LastSnapshotNr` — so P8's own requirement (HMF and GSMF at z=0, z=1, z=2, six separately-named figures, one `--output-dir` per snapshot) has **not** been produced. It also ran without a real shin-uchuu plot profile — `models/sage16/plots/profiles/shin-uchuu_plot_profile.yaml` did not exist yet, so every figure used its mini-millennium-shaped hard-coded axis fallback rather than anything tuned to Shin-Uchuu's resolution (see the profile-fix note below). That profile now exists; the outstanding work is re-running the full plot suite with it and then P8's three-epoch check specifically — tracked as the next session's task in `MIMIC-DEVELOPMENT-PATHWAY.md`.
>
> **The production output has been transferred back to the Mac Studio.** `rsync -aP` from `nt.swin.edu.au:~/Science/mimic/output/sage16-shin-uchuu/` to `/Volumes/LaCie/data/shin-uchuu/output/sage16-shin-uchuu/`: **543,411,126,482 B (≈543.4 GB) transferred, 39 files, at the observed ≈100–110 MB/s per-file rate (≈85–90 min total)** — 10 `model_NNN.hdf5` + the `model.hdf5` master, `metadata/` (schema, run YAML, `a_list`, `simulation_info.yaml`, version info), and the 22 already-generated plot PNGs. Verified present and complete at the destination (11 `.hdf5` files, full `metadata/`). Plotting does not need the 2 TB input tree data, only this output directory and its `metadata/`, so no snapshot data was transferred.

> ### ⛔ P8 FAILED THE SCIENCE CHECK 2026-09-09 — the dataset is wrong at z=0 and must be re-converted
>
> **The validation pass did its job.** P8's first figure, `BaryonFraction`, does not saturate at the cosmic baryon fraction as it must by construction — it climbs to 0.35 against a cosmic 0.17. Root cause: **`fix_flybys()`** (`src/io/vertical/ctrees/ctrees_utils.c:318-412`), the Consistent-Trees reader convention this converter replicates bit-for-bit by design (`scripts/convert/fixups.py:262`, and `HORIZONTAL-HDF5-FORMAT.md` → Ordering Contracts item 5). At each forest's maximum snapshot it demotes every FoF central except the single most massive one to a satellite of that survivor. Consistent-Trees forests are rooted at z=0, so for this dataset that is **snapshot 69 and only snapshot 69** — confirmed by direct scan of `/halos/MostBoundID` in all 70 files (78,405,836 negated at snapshot 69, zero everywhere else) and by the conversion report's own per-snapshot `flyby_demotions`.
>
> On micro-Uchuu this convention costs ~10–25% of Type-0 halos per mass bin and was a documented, accepted divergence. On **Shin-Uchuu it is catastrophic**, because its forests percolate at 8.97 × 10⁵ Msun/h: the percolation super-forest collapses into a single FoF group holding **103,362,317 of the 313,317,969 z=0 galaxies — 33.0% of the box**. The z=0 central population drops to exactly one halo per forest (166,547,771 against a correct ≈244,953,607), the Type-0 halo mass function truncates ~2 dex, nine z=0 figures are invalid, and `sage16`'s infall budget (`sage_prepare_infall_budget.c:110`) subtracts a whole forest's baryons from one halo's `f_b × Mvir`, after which `sage_apply_infall.c:60-91` strips the survivor's ejected and hot gas.
>
> **Everything except snapshot 69 is correct** — but the converter has no per-snapshot re-entry point, and the production run's `--consume-intermediates` deleted every Phase-3-and-later intermediate, so this is a **full re-conversion**. The 11.61 TB source is gone locally (consumed batch-by-batch by design) but intact on NT, which is where it is re-transferred from — the conversion itself runs here on the Mac Studio (addendum D5).
>
> **The plan is [`SHIN-UCHUU-FLYBY-DEFECT-ADDENDUM.md`](SHIN-UCHUU-FLYBY-DEFECT-ADDENDUM.md)** — the evidence, the decisions (delete `fix_flybys` outright; keep `micro-uchuu-ascii`; bump the format to `format_version = 2` with v1 rejected outright; **re-convert here on the Mac Studio by exactly this document's route**, an NT-side conversion having been proposed and rejected by the owner on queue-stability grounds; **attempt P6 locally, with the host chosen at R11 on measured evidence rather than on P5's projection**; keep four transfer batches; and three open owner decisions, including whether to start keeping additional source columns), the R1–R15 sequence, the measured costs, and a Definition of Done numbered 0–11. **That addendum supersedes this document's P1–P9 outcome status** — the Definition of Done below, and the records of P6, P7 and P8 as complete, are **v1 historical evidence, not current completion**. **What this document still owns and what you should keep using it for**: the measured source data, the conversion algorithm, the storage and memory envelopes, every flag semantic and footgun, the operational detail of each stage, and its reusable validation snippets. The remediation deliberately takes the same route this document describes, so all of that applies directly. P6 and P7's measured results stand as evidence that the *pipeline* works end to end at this scale — they were run against a structurally wrong z=0 slab, so their science output is void, but their memory, wall-clock and range measurements are not.
...
| **P6** | **Production `sage16` run**, against the **P3b-verified** dataset. **v1 record: relocated to NT 2026-09-04.** For the v2 run the host is chosen at **R11 on measured evidence** (addendum D7) — a local attempt first, NT as the fallback, because the v1 run measured 507.4 GB against 549.8 GB of physical memory. On the NT branch it reads the dataset **P9 copies there first**. Build and run commands below describe the Mac-Studio-local case; the NT equivalent (paths, partition, job submission) is next session's work. `MODEL`/`SIMULATION` are compile-time (C11). Run detached through a wrapper that **records the true exit code** — P7 must not start on a failed run. Projected peak **≈639–697 GB measured** (superseding the ≈476.6 GB pre-measurement projection — see P5 above) | **≈8–16 h (estimate; the rehearsal wall clock is not committed)** | ⚠️⚠️ **the maximum-memory task of the whole sequence** | YES |
...
| **P9** | **Copy the production dataset** to `/fred/oz214/dcroton/shin-uchuu/snapshot-trees/` — 2.2520 TB measured (supersedes the ≈2.31 TB pre-run figure) at ≈110 MB/s. **Moved ahead of P6 2026-09-04**: this is no longer a post-hoc archive step, it is how the dataset gets to NT for P6 to read. `rsync -aL`, not `--checksum` (same reasoning as "Transfer" above), reverse direction from P1 | ≈5.7 h, background | low | **Now YES for this run — P6 reads what this step copies** |
...
**Roughly 1–2 weeks in total; the machine-bound stretch is P1 → P6, about 4–7 days.** The two maximum-memory moments, in order of severity, are P6 (**≈476.6 GB was the pre-measurement projection here — superseded by P5's real ≈639–697 GB figure, see the P5 callout above; this is exactly why P6 relocated to an NT large-memory node instead of running on the box this section describes**) and P2's `links` stage (≈225–235 GB projected, to be re-derived at P5 before it is trusted).
...
**Step 1 — preflight the volume, and record what it says.** The storage envelope projects a **6.89 TB peak against a 7.0 TB policy ceiling**, binding at scatter through the staged source batch, so **require ≥ 7.0 TB free on the workdir volume before the first batch**. **This 7.0 TB figure is a cold-start-only preflight floor, not an invariant to re-enforce every batch** — measured on the production run 2026-08-29 to 2026-09-03: worker scratch accumulates across all batches and is only deleted once, by `sort`, after `finalize`, so free space legitimately trended down through the whole batch loop and was observed as low as **~5.2 TB** between batches with no problem, well under the 7.0 TB figure but nowhere near actually dangerous. Use a **1.0 TB true-danger floor** for the between-batch re-check instead (what the production run used), and reserve 7.0 TB for the one-time cold-start check only:
...
**Non-finite values are a different finding and are not a range question.** The rehearsal measured zero of them over 406,668,896 halos; any at production points at the producer's `Spin` normalisation (`docs/dev/HORIZONTAL-HDF5-FORMAT.md` records that zero-mass halos' components are carried unnormalised), not at the declared bound. Investigate rather than widen.
...
- The deferred mitigation is worth **≈39.7 GB at the projected slab**: the compact `{int32_t Len, int32_t NextProgenitor}` projection saves 80 B/halo ≈ 28.4 GB (470.4 → ~442.0 GB), and also releasing the retained generation's two int64 identity arrays and its `HorizontalHaloAux` (32 B/halo, 11.3 GB) reaches **~430.7 GB** (`:208`). **The credit is 112 B/halo (80 + 32) and therefore scales with N** — 112 B × 3.546 × 10⁸ = 39.7 GB reproduces the recorded figure, but it is not a fixed 39.7 GB at a larger slab. That is what sets the upper bound below.
...
#    horizontal runs are HDF5-only (C10).
...
**P7 does not start until that file reads `0`.** A failed run leaves the partitions it had already finished **intact and complete on disk**: `bye()` unlinks only the partition in flight, "so a crash never leaves partial output files behind **and never deletes completed ones**" (`src/core/vertical_driver.c:42-46`, `vertical_driver_remove_incomplete_outputs()` at `:59-65`). **For a horizontal run — which P6 is — the equivalent lives in `src/core/horizontal_driver.c:81-96`**: two independently-armed `bye()` slots, INFLIGHT for the partition being written (released the moment that file closes cleanly, so a completed snapshot's output survives a later failure) and MASTER for the run's master file. Every surviving file is therefore individually well-formed, and no per-file check distinguishes that set from a complete run's — which is why P7 asserts the exact expected snapshot set as well as the exit code. Diagnose from `run.log` and re-run: horizontal runs are not resumable (C10), so a failed run is a full restart, not a continuation.
...
3. `output.snapshot_list` in the run file is the set P8 actually needs — see P8. **The operator decision is settled** (nearest-redshift pair 52/43, committed to all four run files 2026-09-04, commit `06d81228`); this check just confirms the run file actually used still carries it. Output snapshots are frozen at launch and the run is not resumable (C10), so a missing snapshot means re-running the whole job.
4. The box is otherwise idle. **≈476.6 GB was the pre-measurement projected peak here — superseded by P5's real ≈639–697 GB figure** (see the P5 callout above), which is why this run no longer targets the Mac Studio's ~540 GB available at all. On whichever NT node actually runs it, confirm its installed memory clears the real figure with comparable headroom before launching, and that the node is otherwise idle.
...
**The scan.** A horizontal run writes **one `model_NNN.hdf5` per requested output snapshot, carrying that snapshot and nothing else**, where `NNN` is the snapshot *number* rather than a dense index — `horizontal_output_partition_output_id()` returns `MimicConfig.ListOutputSnaps[partition]` (`src/core/vertical_driver.c:608-617`), and the filename is formatted at `src/io/output/util.c:36`, alongside the master `model.hdf5` (`src/io/output/master_hdf5.c:36`). Galaxies live in the compound dataset `SnapNNN/Galaxies` (`plot/mimic-plot/hdf5_reader.py:32`, `:42-44`).
...
The expected set above is the shipped list. **Settled 2026-09-04 and committed (`06d81228`)**: all four `shin-uchuu` run files now request `[69, 52, 43, 40, 20, 10, 5, 2, 1, 0]`, the 52/43 pair being the nearest-redshift choice for z = 1 and z = 2. **Copy it from the run file P6 actually used** rather than from here, so the assertion tracks the file rather than this document.
...
**`--output-dir` per snapshot, because the figure filenames carry no snapshot number.** Both figures save under a fixed name — `save_and_close_figure(fig, output_dir, "HaloMassFunction", …)` at `models/sage16/plots/figures/halo_mass_function.py:111` and `"StellarMassFunction"` at `models/sage16/plots/figures/stellar_mass_function.py:233-234`. Pointed at one directory, each iteration overwrites the last, and after the loop **only snapshot 43's two PDFs exist** while the operator believes six were produced — destroying the z = 0 and z = 1 checks, which are the point of the whole pathway. `--output-dir` is a real flag (`mimic-plot.py:1020-1022`, documented at `plot/mimic-plot/README.md:118`) and the directory is created if absent (`os.makedirs(..., exist_ok=True)`, `:1215-1216`, `:1224`), so a per-snapshot subdirectory is all that is needed. The assertion block above is the check that this actually happened; a bare eyeball of the last run's summary cannot distinguish six figures from two.

**The assertion is name-exact, fresh-root-guarded, and exits non-zero — deliberately, on all three counts.** A recursive PDF count under `$PLOTROOT` proves nothing: six leftovers from an abandoned attempt satisfy it while the current run wrote nothing, which is exactly the failure the check exists to catch. So the block refuses to start on a non-empty root and then tests the six exact paths by name. Those names are fixed and predictable: `save_and_close_figure()` writes `os.path.join(output_dir, f"{filename}{output_format}")` (`plot/mimic-plot/output_utils.py:248`), `--format` is passed through verbatim with no dot inserted (`mimic-plot.py:1023`, `:1387`), and `--output-dir` becomes the plots directory itself with no snapshot subdirectory added (`:1216`) — so `--format=.pdf` gives `$PLOTROOT/snap<N>/HaloMassFunction.pdf` and `.../StellarMassFunction.pdf` and nothing else. The final `exit 1` matters because this block is the guard on the overwrite hazard above: `test … && echo … || echo …` returns success on both branches, so a check written that way announces the failure and still reports OK to anything reading its status.

**`--snapshot-plots`, because omitting it doubles the read on the largest snapshot.** With neither tier flag given the parser enables **both** tiers (`mimic-plot.py:1055-1057`). Neither requested figure is an evolution plot, but `generate_evolution_plots()` does not learn that until after it has read the data: with `--snapshot` set it selects that same single snapshot (`:1422-1427`), calls `read_data()` on it (`:1474`), and only then filters against `--plots` and finds nothing to draw (`:1505`). On a production snapshot that is a large read performed twice for no output. `--snapshot-plots` (`:1005-1007`) confines the run to the snapshot tier and skips the second read entirely (`:1582-1591`).

**Corrected 2026-09-08 — this paragraph's original claim was wrong, and it mattered.** `configure_plot_profile()` stacks `plot/mimic-plot/profiles/default.yaml` → the model default → a simulation-level `plot_profile.yaml` → a `<simulation>_plot_profile.yaml` under the model → any `plotting.profile` in the run file (`mimic-plot.py:309-339`). When P6's sbatch job first ran its plotting step, only the first two existed for this pair, and the profile's `mode:` key (`exploration`/`validation`) is **pure documentation with zero code effect** — `get_profile_axes()` and `configure_plot_profile()` never read it. So the actual fallback was not "auto-scaled axes"; it was each figure's hard-coded default axis window (`halo_mass_function`'s `PLOT_XLIM = (10.0, 15.0)`, `stellar_mass_function`'s `(8.0, 12.5)`, etc.) — numbers shaped for mini-millennium/millennium's much coarser particle mass. Since Shin-Uchuu resolves halos roughly 3 dex further down, those defaults silently clipped the low-mass regime Shin-Uchuu exists to resolve, in every figure that plotted a mass axis. **Fixed 2026-09-08**: `models/sage16/plots/profiles/shin-uchuu_plot_profile.yaml` and `shin-uchuu-ascii_plot_profile.yaml` now exist, with axis overrides derived from the measured particle-mass ratio (see the profile files' own comments for the full derivation). A pre-existing, unrelated bug was found and fixed in the same pass: `mini-millennium_plot_profile.yaml`/`millennium_plot_profile.yaml` (and the `sham`/`halos-only` equivalents) used list-form `xlim: [a, b]` for these same two figures, a key `get_profile_axes()` never reads (it reads scalar `xmin`/`xmax`) — those profiles' overrides had been silent no-ops for the life of the repo and are now corrected. See the `mimic-plots-and-analysis` skill for the current (fixed) convention.

**~~The shipped `output.snapshot_list` cannot produce z = 1 or z = 2, and this must be fixed before P6, not at P8.~~ DISCHARGED 2026-09-04 (`06d81228`)** — `models/sage16/input/sage16_shin-uchuu.yaml:19` now requests `[69, 52, 43, 40, 20, 10, 5, 2, 1, 0]`; do not "fix" it again. The analysis below is retained because it is how the 52/43 pair was chosen. It originally read: `sage16_shin-uchuu.yaml:19` requests `[69, 40, 20, 10, 5, 2, 1, 0]`. Against `simulations/shin-uchuu/shin-uchuu.a_list` (70 entries, snapshot *n* = line *n+1*):
...
| # | Constraint | Evidence |
|---|---|---|
| **C4** | **File IDs must be contiguous from 0, *and* the file count must be a perfect cube.** `read_locations()` asserts `max_fileid + 1 == numfiles`, then that `round(cbrt(numfiles))³ == numfiles`. **2,744 = 14³**, which is why any subset must close file coverage rather than drop a file | `src/io/vertical/ctrees/ctrees_utils.c:236-246` |
| **C5** | **A forest's vertical-driver reader memory has a floor set by its *tree count*, independent of halo count.** `load_unit_ctrees_ascii()` preallocates `1000 × ntrees` records in both `halo_data` and `additional_info` — 152,000 B per tree. The percolation super-forest's 104,845,278 tree roots alone demand ≈15.9 TB | `src/io/vertical/read_ctrees_ascii.c:647-655` |
| **C6** | **The vertical driver loads a forest as one in-memory unit**, so the super-forest can never be read vertical. This is why the horizontal driver exists, why the rehearsal subset excluded that forest, and why a production-scale topology cross-check is impossible in principle rather than merely expensive | C5 quantifies it |
...
| **C10** | **Horizontal runs are HDF5-only, serial-only, and cannot use `--skip`.** All three are config-time rejections | `src/core/read_parameter_file.c:1475-1488` |
...
> **⛔ v1 HISTORICAL EVIDENCE, NOT CURRENT COMPLETION (2026-09-09).** Every item below was satisfied by the v1 conversion and the v1 `sage16` run, and every one of them is now void for acceptance purposes: the dataset those runs produced is structurally wrong at z=0 (see the "P8 FAILED THE SCIENCE CHECK" callout above). The live acceptance criteria are the twelve items in [`SHIN-UCHUU-FLYBY-DEFECT-ADDENDUM.md`](SHIN-UCHUU-FLYBY-DEFECT-ADDENDUM.md) → "Definition Of Done". This list is kept because the *measurements* behind each item — wall clock, peak RSS, ranges, counts — remain valid evidence about the pipeline, and because the v2 run has to satisfy the same items again.

1. ✅ **DONE.** 70 HDF5 snapshot files produced and validated (halo count, adjacency, round-trip progenitor check, FoF chain integrity, NextProgenitor same-file scope, identity uniqueness, Len non-negative)
2. ✅ **DONE.** **Topology cross-check passes on micro-Uchuu** by stable halo identity (the converter acceptance gate, completed long before the production run)
3. ✅ **DONE.** The conversion report exists and the shin-uchuu identity multiplier is set from its measured counts
4. ✅ **DONE.** `simulations/shin-uchuu/` package registered and building clean with the horizontal reader
5. ✅ **DONE 2026-09-07.** Mimic runs `sage16` on Shin-Uchuu end-to-end: no assertion failures, no broken links, no memory errors — job `16267967` `COMPLETED`, 472.3 GB peak RSS, 17h04m43s, all ten requested snapshots emitted; P7's post-run `deltaMvir` scan (2026-09-08) confirms no broken output
6. ⬜ **OPEN.** HMF and GSMF plots produced and sanity-checked at z=0, z=1, z=2 — **six figures, two per snapshot, each snapshot written to its own `--output-dir`** (P8: the figure filenames carry no snapshot number, so a shared directory silently leaves only the last snapshot's pair). The default plotting step so far only covers z=0 (18 figures) plus the 4 evolution plots — the three-epoch check itself has not been run. A real shin-uchuu plot profile now exists (see "P6 attempt 2 succeeded" above); re-running the plot suite with it, then this specific check, is the next session's task
