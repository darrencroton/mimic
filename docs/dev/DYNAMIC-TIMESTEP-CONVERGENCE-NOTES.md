...
7. **Widen Consistent-Trees-sourced simulation catalog properties to `double`** (`micro-uchuu-hdf5`, `uchuu`, `micro-uchuu-ascii`). **Investigated, not applied — no real gain.** See the precision section below.
...
- **Simulation catalog properties** (`simulations/<SIM>/halo_properties.yaml`): all declare `float` for masses/positions/velocities/spins uniformly across every package. For `lhalo_binary`-sourced simulations (`mini-millennium`, `millennium`, `micro-uchuu`, `mini-uchuu`) this is correct and lossless — the on-disk format is single precision. For **Consistent-Trees-sourced simulations** (`micro-uchuu-hdf5`, `uchuu`, `micro-uchuu-ascii`): the reader code reads these fields as `double` C locals (`CT_ASSIGN_SINGLE(mvir, double, halos, Mvir)`), which initially looked like an avoidable precision loss. **Investigated and reverted — not a real gain.** Two things rule it out: (1) the actual catalog values (checked directly against the source Uchuu ASCII/HDF5 files) are written to ~7 significant figures by Rockstar/Consistent-Trees itself (e.g. `Mvir=3.10900e+14`, `Rvir=1375.334000`); a `double` container over that data carries no more real information than `float`. (2) even setting that aside, the data actually flows through a shared, hand-written `struct halo_data` (`src/io/tree/ctrees/ctrees_compat.h`) that hardcodes `float` and is deliberately kept byte-compatible with sage-model's vendored Consistent-Trees parsing code for easy upstream re-sync — the HDF5/ASCII `double` read gets narrowed to `float` there regardless of what `halo_properties.yaml` declares downstream, so a YAML-only edit would have been a no-op. Widening `ctrees_compat.h` itself was considered and rejected given (1) already shows there's nothing to gain.
